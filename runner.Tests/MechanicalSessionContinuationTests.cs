...
public sealed class MechanicalSessionContinuationTests
{
    ...
    [Fact]
    public void Resumed_fallback_is_typed_on_the_durable_completion_plane()
    {
        var classified = ExecutionOutcomeAdapter.Classify(new ExecutionRawFacts(
            "run-mechanical", ExecutionAttemptKind.Coding, LaunchFailed: true));
        var fallback = MechanicalRoundFallbackPolicy.AsTypedDecision(
            classified, "semantic-conflict");
        fallback = RemoteTaskRunner.WithDurableOutput(fallback, WorktreeTeardownResult.NoWork);
        var payload = RemoteTaskRunner.BuildDurableCompletion(
            new RunOutcome(RunOutcomeKind.MechanicalFallback, "semantic-conflict"),
            fallback, WorktreeTeardownResult.NoWork, null, null, "host");

        Assert.Equal(ExecutionOutcomeKind.MechanicalFallback, fallback.Outcome);
        Assert.Equal(ExecutionRecoveryAction.StartFreshAttemptFromSalvage, fallback.RecoveryAction);
        Assert.False(fallback.ConsumesProductDefectBudget);
        Assert.Equal(ExecutionOutcomeKind.MechanicalFallback.ToString(), payload.Outcome);
        Assert.Equal(payload.Outcome, Assert.IsType<ExecutionOutcomeDecision>(payload.OutcomeDecision).Outcome.ToString());
    }
