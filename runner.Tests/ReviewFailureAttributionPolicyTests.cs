...
public sealed class ReviewFailureAttributionPolicyTests
{
    ...
    {
        var attribution = ReviewFailureAttributionPolicy.Attribute(
            commandSucceeded: false,
            CapturedVerifyOutput,
            string.Empty);

        Assert.Equal(ReviewFailureAttribution.ProductOwned, attribution);
    }
    ...
    public void Signature_in_the_assertion_message_of_a_failed_test_is_a_product_failure()
    {
        ...
        Assert.Equal(
            ReviewFailureAttribution.ProductOwned,
            ReviewFailureAttributionPolicy.Attribute(commandSucceeded: false, output, string.Empty));
    }
