...
async function stub(page: Page) {
  ...
  const modelRoutes = {
    ...
    routes: [
      ...
      {
        fromCliType: 'claude', fromModel: 'claude-opus-5', fromThinkingLevel: 'medium',
        toCliType: 'codex', toModel: 'gpt-5.6-sol', toThinkingLevel: 'medium',
        source: 'catalogue', catalogueVersion: 'token-economy-0.3.4+2026-07-24',
        fromInputPerMTok: 15, fromOutputPerMTok: 75,
        toInputPerMTok: 2, toOutputPerMTok: 10,
        capabilityClass: 'tier-2', reroutable: true,
...
test.describe('CLI Management restructure (AGT-2101)', () => {
  ...
  test('rail entry + page heading both read "CLI Management"; catalog is rows leading with what is present', async ({ page }) => {
    ...
    page.on('console', (message) => {
      if (message.text().includes('NG0955')) duplicateKeyWarnings.push(message.text());
    });
    await openHome(page);
    ...
    await expect(fallbackRoutes).toContainText('claude-opus-5 medium');
    await expect(fallbackRoutes).toContainText('gpt-5.6-sol high');
    await expect(fallbackRoutes.locator('tbody tr')).toHaveCount(2);
