...
async function applyPreDeliveryCardLayout(page: Page): Promise<void> {
  await page.addStyleTag({ content: `
    .workbench-overview__row {
      grid-template-columns: minmax(0, 1fr) auto !important;
      align-items: center !important;
      gap: var(--studio-spacing-4) var(--studio-spacing-6) !important;
    }
    .workbench-overview__row-main { display: block !important; }
    .workbench-overview__meta { margin-top: var(--studio-spacing-2) !important; }
    .workbench-overview__references > span:first-child,
    app-copyable-task-key { display: none !important; }
    .workbench-overview__excerpt {
      display: block !important;
      max-width: 76ch !important;
      margin-top: var(--studio-spacing-2) !important;
      overflow: visible !important;
      -webkit-line-clamp: unset !important;
    }
    .workbench-overview__footer { display: none !important; }
  ` });
  await page.locator('[data-testid^="workbench-overview-item-"]').evaluateAll(rows => {
    for (const row of rows) {
      const actions = row.querySelector('[data-testid^="workbench-overview-actions-"]');
...
test('Dossier cards keep long and short summaries readable across widths and themes', async ({ page }, testInfo) => {
  ...
  await page.route(
    `**/api/projects/${encodeURIComponent(PROJECT)}/workbenches/isolated-execution`,
    route => json(route, {
      workbench: {
        ...visualItems[0].workbench,
        decision: null,
        documentation: null,
      },
      html: '<!doctype html><html><body><main><h1>Isolated execution</h1></main></body></html>',
      branch: 'task/agt-2783',
      revision: 'agt2783evidence',
      workingTreeModified: false,
      fingerprint: 'a'.repeat(64),
    }),
  );
  await page.route(
    `**/api/projects/${encodeURIComponent(PROJECT)}/workbenches/AGT-W51/references`,
    route => json(route, {
      projectName: PROJECT,
      workbenchKey: 'AGT-W51',
      workbenchId: 'isolated-execution',
      legacyTaskKeys: [],
      items: [],
    }),
  );
  ...
  for (const width of [1536, 900]) {
    ...
    if (phase === 'before') {
      await applyPreDeliveryCardLayout(page);
      for (const theme of ['light', 'dark'] as const) {
        await setTheme(page, theme);
        await page.screenshot({
          path: evidencePath(
            testInfo,
            `agt-2783--dossier-cards--before--${width}px--${theme}--composite.png`,
          ),
          fullPage: true,
        });
      }
    ...
    if (width === 1536) {
      const reviewButton = longCard.getByTestId(
        `workbench-overview-open-${PROJECT}-isolated-execution`,
      );
      await reviewButton.click();
      const inlineViewer = longCard.getByTestId(
        `workbench-overview-inline-${PROJECT}-isolated-execution`,
      );
      await expect(inlineViewer).toBeVisible();
      expect(await inlineViewer.evaluate((viewer, card) => {
        const viewerWidth = viewer.getBoundingClientRect().width;
        const cardWidth = (card as HTMLElement).getBoundingClientRect().width;
        return viewerWidth >= cardWidth * 0.9;
      }, await longCard.elementHandle())).toBe(true);
      await reviewButton.click();
      await expect(inlineViewer).toHaveCount(0);
    }
