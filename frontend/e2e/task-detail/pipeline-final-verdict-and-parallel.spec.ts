...
test.describe('Pipeline: parallel aspects + orchestrator final verdict', () => {
  ...
  test('captures aspect, sentinel, and tab-strip evidence at desktop widths', async ({ page }) => {
    ...
    for (const width of [1280, 1920]) {
      ...
      await pipeline.screenshot({ path: path.join(RESULTS_DIR, `pipeline-before-${width}.png`) });
      ...
      await page.getByTestId('overview-pipeline').screenshot({
        path: path.join(RESULTS_DIR, `pipeline-after-${width}.png`),
      });
      ...
      await tabbar.screenshot({ path: path.join(RESULTS_DIR, `tab-strip-before-${width}.png`) });
      ...
      await page.getByTestId('studio-tabbar').screenshot({
        path: path.join(RESULTS_DIR, `tab-strip-after-${width}.png`),
      });
      ...
      await overlay.screenshot({ path: path.join(RESULTS_DIR, `log-before-${width}.png`) });
      ...
      await overlay.screenshot({ path: path.join(RESULTS_DIR, `log-after-${width}.png`) });
