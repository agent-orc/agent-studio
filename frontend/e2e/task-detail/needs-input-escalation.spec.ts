...
async function installRoutes(page: Page): Promise<void> {
  ...
  await page.route(new RegExp(`/api/tasks/${JOB_ID}/runs(\\?|$)`), route => json(route, {
    ...
    runs: [
      ...
      },
      {
        ...
        status: 'completed', cli: null, model: 'gpt-6-sol', thinkingLevel: 'high', exitCode: 0,
  ...
  await page.route(new RegExp(`/api/tasks/${JOB_ID}/session-events(\\?|$)`), route => json(route, {
    events: [{
      ts: '2026-09-11T14:40:00.000Z', kind: 'continue', cli: 'codex', inputSessionId: 'session-1',
      capturedSessionId: 'session-1', resumed: true, reason: null,
    }],
...
test('a parked card says why it is parked, above the derived escalation headline', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('parked-session-model')).toContainText('codex');
