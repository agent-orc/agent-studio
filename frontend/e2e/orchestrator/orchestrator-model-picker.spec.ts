...
async function stubWorkspace(page: Page, sent: Record<string, unknown>[] = []) {
  await page.route(/\/api\//, route => {
    ...
    if (requestPath.endsWith('/workbenches')) body = JSON.stringify({ projectName: PROJECT, count: 0, items: [] });
  ...
  await page.route(/\/api\/projects\/?$/, route => route.fulfill({
    status: 200, contentType: 'application/json', body: JSON.stringify([{ id: PROJECT,
      displayName: PROJECT, shortCode: 'MP', workspaceId: 'workspace-1', storageLocation: `/tmp/${PROJECT}`,
      archived: false, urls: [{ id: 'preview', label: 'Preview', url: 'https://example.test' }] }]),
  }));
...
test('full live GPT picker persists across Board and Task contexts', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('chat-context-attachment-automatic-context')).toContainText('Deck · ~1.6k');
  ...
  await expect(page.getByTestId('chat-context-attachment-automatic-context')).toContainText('preview · ~1.6k');
  await expect(page.getByTestId('cac-model-selector-trigger')).toContainText('gpt-5.6-sol');
  ...
  await expect(page.getByTestId('chat-context-attachment-automatic-context'))
    .toContainText(`${PROJECT}::task-1 · ~1.6k`);
