...
export class OrchestratorSideSheetComponent implements OnInit, OnDestroy {
  ...
  /**
   * Canonical active-tab context, derived by Studio and projected into the
   * automatic CAC context chip.
   */
  ...
  readonly cacContextAttachments = computed<readonly ChatContextAttachment[]>(() => {
    const explicit = this.contextAttachments();
    const automaticIncluded = this.contextKind() === 'task' || !this.contextDismissed();
    const estimatedTokens = (automaticIncluded ? 1_600 : 0)
      + explicit.reduce((sum, item) => sum + item.estimateTokens, 0);
    const automatic = this.automaticContextPresentation();
    const items: ChatContextAttachment[] = automaticIncluded
      ? [{
          id: 'automatic-context',
          label: `${automatic.key?.trim() || automatic.label} · ${formatCompactTokens(estimatedTokens)}`,
          hint: `Current ${automatic.typeLabel.toLocaleLowerCase()}: ${automatic.label}. `
            + `${formatContextEstimate(1 + explicit.length, estimatedTokens)}.`,
        }]
      : [];
    return [
      ...items,
  ...
  removeContextAttachment(id: string): void {
    if (id === 'automatic-context') {
      if (this.contextKind() === 'project') this.contextDismissed.set(true);
      return;
    }
  ...
  async onSubmit(event: ChatSubmitEvent): Promise<void> {
    const proj = this.effectiveProject();
    if (!proj) return;
    // Snapshot once so a navigation change during send cannot
...
function formatContextEstimate(sourceCount: number, tokens: number): string {
  const sourceLabel = sourceCount === 1 ? 'source' : 'sources';
  return `${sourceCount} ${sourceLabel}, about ${new Intl.NumberFormat('en-US').format(tokens)} tokens`;
}
