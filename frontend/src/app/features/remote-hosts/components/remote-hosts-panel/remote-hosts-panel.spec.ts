...
describe('RemoteHostsPanelComponent', () => {
  ...
  it('shows the Stable release and marks each role that lags it', async () => {
    ...
    // The machine row summarises the same drift, so the table never reads calmer
    // than its rows (R3).
    expect(el.querySelector('[data-testid="remote-host-release-drift"]')?.textContent?.trim())
      .toBe('19d behind');
