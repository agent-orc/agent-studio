...
type ProviderSignInPhase = 'starting' | 'pending' | 'verifying' | 'failed';
...
export class ProviderSignInDialogComponent implements OnDestroy {
  private readonly dialog = inject(ProviderSignInDialogService);
  ...
  readonly phase = signal<ProviderSignInPhase>('starting');
  readonly session = signal<ProviderSignInStartResponse | null>(null);
  readonly detail = signal('Starting provider sign-in on the execution host...');
  ...
  private readonly requestEffect = effect(() => {
    ...
    const key = `${request.hostId}|${request.provider}|${request.sshTarget}|${request.baselineAdvertisedAt ?? ''}`;
  ...
  private launch(request: ProviderSignInTarget): void {
    ...
    this.detail.set(`Starting a host-owned ${this.providerLabel(request)} sign-in on ${request.hostName}...`);
    if (!sshTarget) {
      this.fail('This execution host has no SSH target. Open its setup dialog and configure the host address first.');
      return;
    }

    this.providerAuth.startProviderSignIn(request.hostId, request.provider, sshTarget).subscribe({
      next: session => {
      ...
      error: error => this.fail(errorDetail(error, `${this.providerLabel(request)} sign-in could not be started.`)),
    });
  }

  private poll(request: ProviderSignInTarget, session: ProviderSignInStartResponse): void {
    this.polling = timer(500, 1_000).pipe(
      switchMap(() => this.providerAuth.providerSignInStatus(request.hostId, session.handle)),
      takeWhile(status => status.state === 'pending', true),
    ).subscribe({
      next: status => this.acceptStatus(request, status),
      error: error => this.fail(errorDetail(error, `${this.providerLabel(request)} sign-in status could not be read.`)),
    });
  }

  private acceptStatus(request: ProviderSignInTarget, status: ProviderSignInStatusResponse): void {
    ...
    const label = this.providerLabel(request);
    this.detail.set(`${label} confirmed the login. Waiting for a fresh runner provider probe...`);
    this.polling = this.providerAuth.waitForFreshProbe(
      ...
      next: badge => {
        ...
        this.notifications.success(
          `${label} authentication is available on ${badge.hostName}. Ready cards can resume.`,
          `${label} sign-in complete`,
        );
        this.dialog.refreshHosts();
        this.close();
      },
      error: () => this.fail(`${label} signed in, but a fresh OK provider probe did not arrive in time. Re-probe the host or retry.`),
    });
  }
  ...
  private reset(): void {
    ...
    this.phase.set('starting');
    this.session.set(null);
    this.detail.set('Starting provider sign-in on the execution host...');
    this.copyLabel.set('Copy code');
  }

  providerLabel(request = this.request()): string {
    return request?.provider === 'claude' ? 'Claude' : 'Codex';
  }
}
