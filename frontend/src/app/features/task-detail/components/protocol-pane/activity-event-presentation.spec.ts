...
// AGT-2793, operator report: "a block of runner status rows... repeats the
// same visual label (RUNNER) four times with a different meaning each time...
// [and] puts a trace button on every row, even though there are only six
// facts". The library renders one `<li>` per `system.status` event with no
// grouping hook, so the fix collapses the run into one event at the
// projection layer; the compatibility directive (activity-event-presentation
// .directive.ts) turns `runnerGroupRows` into the nested per-fact disclosure.
