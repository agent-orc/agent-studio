...
export class OverviewPaneComponent {
  ...
  readonly tokenRunCount = computed(() => this.pipelinePoll.pipeline()?.tokensByModel?.runs.length ?? 0);
  readonly hasMultipleTokenRuns = computed(() => this.tokenRunCount() > 1);
  pipelineTotalLabel(): string { return this.hasMultipleTokenRuns() ? 'This run · pipeline total' : 'All runs · task total'; }
  pipelineTotalScope(): string { return `Run #${this.pipelineAttempt()} incl. pre/post/review steps`; }
  pipelineTotalScopeTooltip(): string {
    return this.hasMultipleTokenRuns()
      ? `This total includes the core agent run and every pre, post, and review step in Run #${this.pipelineAttempt()} only.`
      : 'This task has one run; the total includes its core agent run and every pre, post, and review step.';
  }
