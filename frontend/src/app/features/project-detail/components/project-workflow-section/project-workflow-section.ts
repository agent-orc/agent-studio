...
import { laneSentence } from '../../../../models/lane-presentation';
...
export class ProjectWorkflowSectionComponent implements OnInit {
  ...
  // Name, glyph, role sentence, and tone all come from the lane presentation
  // (AGT-2715). This page used to keep its own role copy, which is how it ended
  // up saying "Awaiting human review." for the lane every other surface called
  // something else.
  readonly lanes: readonly WorkflowLaneRow[] = SORTABLE_LANES.map((lane) => ({
    state: lane.state,
    label: lane.label,
    icon: lane.icon,
    role: laneSentence(lane.state),
  }));
