...
public sealed class ReviewPlaneBudgetProbe
{
    ...
    private readonly Func<int> _hostCpuCount;
    private readonly Func<string> _readCpuMax;
    private readonly Func<string> _readCpuStat;
    ...
    public ReviewPlaneBudgetProbe()
        : this(HostCpuCount, ReadUnitCpuMax, ReadUnitCpuStat)
    {
    }

    internal ReviewPlaneBudgetProbe(
        Func<int> hostCpuCount,
        Func<string> readCpuMax,
        Func<string> readCpuStat)
    {
        _hostCpuCount = hostCpuCount;
        _readCpuMax = readCpuMax;
        _readCpuStat = readCpuStat;
    }
    ...
    /// <summary>
    /// Reads only the current role quota for admission. Unlike <see cref="Sample"/>,
    /// this has no delta or alarm state, so frequent claim polls cannot shorten
    /// the throttling measurement window. Reading <c>cpu.max</c> on every poll
    /// also makes a live systemd quota change effective before the next
    /// capability advertisement.
    /// </summary>
    public double ReadRoleQuotaCores()
    {
        var hostCores = Math.Max(1, _hostCpuCount());
        var cpuMax = ParseCpuMax(_readCpuMax());
        return Math.Min(hostCores, cpuMax.QuotaCores ?? hostCores);
    }
    ...
    private static string ReadUnitCpuMax()
    {
        var unitCgroup = ResolveUnitCgroupDirectory();
        return ReadOrDefault(
            unitCgroup is null ? null : Path.Combine(unitCgroup, "cpu.max"),
            "max 100000");
    }

    private static string ReadUnitCpuStat()
    {
        var unitCgroup = ResolveUnitCgroupDirectory();
        return ReadOrDefault(
            unitCgroup is null ? null : Path.Combine(unitCgroup, "cpu.stat"),
            string.Empty);
    }
