...
public sealed class RemoteTaskRunner
{
    ...
    /// <summary>
    /// Inventory whatever evidence the stopped run already wrote. The code
    /// handoff still happens before any artifact bytes are sent.
    /// </summary>
        ...
        DurableRunOutbox? outbox)
    {
        ...
        {
            _log($"operator-stop result inventory failed: {ex.Message}");
            return null;
        }
    }
        ...
        CancellationToken daemonShutdown = default)
    {
        ...
        try
        {
            ...
            artifactPlan = await PrepareResultsAsync(taskKey, artifactLimits, outbox, stopRun.Token);
