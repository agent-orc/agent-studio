...
public sealed class RemoteProjectChatRunner
{
    ...
    public async Task<int> RunAsync(RemoteChatWorkItem work, CancellationToken shutdown)
    {
        ...
        try
        {
            ...
            var cliType = string.IsNullOrWhiteSpace(work.CliType) ? "codex" : work.CliType.Trim().ToLowerInvariant();
            ...
            var process = await RunAgentThroughCarAsync(work, cliType, checkout, contextPrompt, timeout.Token, shutdown);
