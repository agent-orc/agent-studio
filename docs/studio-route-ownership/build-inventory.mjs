...
const extras = [
  ...
  // task.service.ts builds these through a `scope === 'code' ? checkout-path
  // : files-path` ternary assigned to a local `base`, which the static
  // extractor cannot evaluate; both branches are real, already-landed
  // routes (backend/Features/Tasks/TaskFilesEndpoints.cs splits them the
  // same way, "Split three mixed contracts before cutover").
  ['GET', '/api/tasks/{taskId}/files/{path*}', 'frontend/src/app/services/task.service.ts:1306'],
  ['GET', '/api/tasks/{taskId}/files/{path*}', 'frontend/src/app/services/task.service.ts:1324'],
  ['GET', '/api/tasks/{taskId}/files/{path*}/history', 'frontend/src/app/services/task.service.ts:1261'],
  ['GET', '/api/tasks/{taskId}/checkout/{path*}', 'frontend/src/app/services/task.service.ts:1306'],
  ['GET', '/api/tasks/{taskId}/checkout/{path*}', 'frontend/src/app/services/task.service.ts:1324'],
  ['GET', '/api/tasks/{taskId}/checkout/{path*}/history', 'frontend/src/app/services/task.service.ts:1261'],
];
...
export function splitQuery(input) {
  ...
  // A call site built its whole query string behind one interpolated
  // variable (e.g. `` `${this.baseUrl}/cases${query}` ``) with no literal
  // `?` in the source. extract-routes.mjs has no way to know the
  // interpolation is query-shaped, so it lands as a trailing `{query}`
  // path placeholder. Treat that placeholder as a dynamic query suffix,
  // not a path segment.
  const querySuffix = input.match(/\{query\}$/);
  if (querySuffix) {
    return { path: input.slice(0, querySuffix.index), fixed: {}, optional: [], dynamic: true };
  }
  const question = input.indexOf('?');
  if (question < 0) return { path: input, fixed: {}, optional: [], dynamic: false };
  ...
  for (const part of raw.split('&')) {
    ...
    // A whole-query interpolation such as `?${params.toString()}` resolves
    // to a single opaque placeholder (e.g. `{toString}`) rather than a
    // literal `key=value` pair. Record it as a dynamic, unnamed query
    // instead of inventing a fake query key from the expression's tail.
    if (/^\{[^{}=]+\}$/.test(part)) {
      dynamic = true;
      continue;
    }
  ...
  return { path: input.slice(0, question), fixed, optional, dynamic };
}
...
function inferredQueryKeys(method, routePath) {
  ...
  if (/\/tasks\/\{taskId\}\/(?:files|checkout)\/\{path\*\}/.test(routePath)) {
    ...
    // only the Task Server-backed files route accepts a caller-selected scope.
    if (routePath.includes('/files/')) add('scope');
...
// A route already "exists" in v1 exactly when the frontend calls a path
// under /api/v1/ today; the backend groups above only ever accept a call at
// the path the frontend actually sends. Earlier revisions of this script
...
function targetRoute(method, routePath, routeClass) {
  ...
  if (isExistingV1(routePath)) return routePath;
...
export function buildInventory(candidatePayload, { backendRoot = path.join(root, 'backend') } = {}) {
  ...
  for (const candidate of [...candidatePayload.routes, ...extras.map(([method, route, evidence]) => ({ method, path: route, evidence: [evidence] }))]) {
    ...
    const current = merged.get(key) ?? { method: candidate.method, path: routePath, evidence: [], fixed: {}, optional: [], dynamic: false };
    ...
    current.dynamic = current.dynamic || query.dynamic;
  ...
  const frontendRoutes = [...merged.values()]
    ...
    .map((route, index) => {
      ...
      const status = routeClass === 'dev-seat' ? 'not-applicable' : isExistingV1(route.path) ? 'exists' : 'must-add';
      ...
      if (route.dynamic) queryVariants.push({ kind: 'dynamic' });
  ...
  const groupMatches = [];
  for (const file of walk(backendRoot).filter((item) => item.endsWith('.cs')).sort()) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const output = `${JSON.stringify(buildInventory(candidatePayload), null, 2)}\n`;
