...
- Deterministic acceptance rail: `AcceptanceRailHostedService` repeats the
  ...
  recovery events in the current operator review epoch. Once it is spent, the
  park reason is exactly `automatic recovery budget used: 2/2`. Concept and report-only cards
