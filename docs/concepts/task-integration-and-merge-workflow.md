...
- Execution: both the immediate coordinator and `AcceptedIntegrationWorker` call `MergeIntoDevelopRunner.Run` (`backend/Features/Pipeline/MergeIntoDevelopRunner.cs`). Every git mutation of that run happens in the Studio-owned integration worktree, derived from the repository path and reset per integration, never in the registered developer checkout ([operations/git/integration-worktree.md](../operations/git/integration-worktree.md), AGT-2832). Immediately before any merge or release gate, the runner fetches the configured integration branch and fast-forwards a stale local branch. It leaves a local-ahead branch intact and reports a divergent branch without overwriting either tip. Delivery integration uses the ordered stages below. Rebase is the final mechanical fallback, not the integration strategy.

| Stage | Attempt | Success | Failure evidence |
| --- | --- | --- | --- |
| Direct merge | `git merge --no-ff` of the reviewed delivery into the synchronized integration branch | Original delivery SHAs are preserved in a merge commit | Count and names of unmerged files |
| Mechanical merge | One three-way `ort` merge with recorded `rerere` resolution | Original delivery SHAs are preserved in a merge commit | Files still unmerged after the mechanical resolution attempt |
| Rebase fallback | One `rebase --rebase-merges` in a disposable detached worktree, with `rerere` and autostash disabled | Accepted only with a one-to-one old/new delivery commit mapping, then merged normally | Stopped commit and position, conflicted files, or the failed mapping check |

The fallback cleanup removes both the temporary directory and its Git worktree registration before the target repository is mutated. If all three stages fail, the integration record keeps the structured stage outcomes, integration tip, delivery SHA, and a capped conflicted-file sample with the uncapped count. The card renders a concise two- or three-line summary without Git's interactive conflict hints.
...
- Conflict recovery: only a delivery that remains textually conflicted after the
  three-stage integration attempt renders **Rebase & retry** next to its red
  ...
  resume its existing delivery ref and return a new fenced result for
  acceptance. The recovery prompt asks for a cleanly integrable state and
  prefers merging the integration branch into the card branch with explicit
  conflict resolution over rewriting delivery history. A passed review receives
  an automatic recovery round while its bounded delivery-chain budget remains.
  Once the budget is spent, the Human Review park names the exact `used/max`
  count instead of exposing the internal `AgentRoundRequired` verdict.
