...
thinking level, prompt source, and log span; the session-event record supplies
the actual CLI when older run projections omit it, and task settings are shown
as pinned values only when they differ. The existing bounded CLI-output
projection feeds the shared conversation renderer, with older lines disclosed
in batches and a link to the complete run log. This adds no filesystem or HTTP
contract.
