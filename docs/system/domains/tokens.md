...
exists is a no-op. The completion report is written only when every required
task and pipeline write succeeds. A failed attempt reports repaired,
skipped-already-correct, and failed keys without writing the completion marker;
the next startup retries the failed items and recognizes successful earlier
writes by their per-entry normalization marker.
