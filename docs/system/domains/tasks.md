...
Task Server store schema 14 is the first combined migration-capable format:
schema 12 owns scoped principals and credentials, schema 13 owns retention and
archive state, and schema 14 owns the legacy migration ledger and artifact
source pointers. A version-13 binary therefore rejects a store already opened
