...
| `db-touch` | `POST /api/_internal/probe` round-trips its sentinel; the endpoint has its own gate (`UpdateService:ProbeEnabled`, open by default where the Stable update contract is installed) and returns 404 otherwise |
...
**Revision (2026-09-17).** The verification matrix is part of the preflight, not only of the post-restart phase. Every phase-6 step declares what it needs from the running instance, and `GET /update/preflight` probes those preconditions before an update is allowed. A step whose endpoint answers 401/403/404 is a durable configuration failure that a restart reproduces, so it refuses the run with an actionable error; every other unmet precondition is reported without refusing, because the update may be the repair and phase 6 owns its own retry budgets. The `db-touch` sentinel no longer depends on a DevTools flag: it has its own gate (`UpdateService:ProbeEnabled`) that is open by default wherever the Stable update contract is installed, while the DevTools SSE stream keeps its separate flag and stays closed on Stable. Driver: run `0649a4e0` (17.09.2026, v0.6.0 to v0.7.0) failed `db-touch` with http=404 on a default Stable after the stack had already been stopped, restored, rebuilt and restarted, and the failure then drove the rollback path underneath the already-running new backend.
...
**Amendment (2026-09-18, delivery-scoped recovery budget).** The bounded integration-recovery loop permits at most two automatic Steer rounds for one fenced delivery chain. The immutable result ref and delivered SHA form the durable chain identifier recorded on every automatic recovery event. Another review of the same delivery shares that budget across review epochs, while a newly published delivery starts a fresh budget. Identifier-less historical events count conservatively. Exhaustion names the usage and short delivery SHA instead of exposing a generic review-verdict reason.
...
## ADR-0074 - The hosted Wiki publishes an accepted revision by fetch and materialize, never by checking out the deployment tree (2026-09-15)

**Decision.** The hosted Wiki serves an immutable, SHA-addressed snapshot of `docs/` that the backend materializes from an accepted integration branch or release revision. One supervised sync fetches the configured remote, resolves the candidate commit, extracts that commit's `docs/` tree into a staging folder, publishes it with a single directory move, and only then swaps one immutable published-revision reference. The deployment repository's working tree is never checked out, reset, or written to. Every Wiki surface resolves against the published commit until the next promotion, and every non-promoting outcome leaves the previous revision online with a typed failure.

**Context.** Hosting the Wiki API is not the same thing as publishing documentation. The control-plane checkout still had to advance to the accepted documentation revision without an operator copying files over SSH, and readers had to keep seeing a coherent tree while it advanced. The obvious mechanism, running `git checkout` or `git reset --hard` in the deployment checkout on a timer, was rejected on three counts: it mutates a working tree that a hosted read session must never be able to commit into, it is not atomic (a reader mid-request observes a partially rewritten `docs/` tree), and it has no rollback beyond a second mutation of the same tree. A parallel question was which revision is publishable at all: an arbitrary active task branch would make the hosted Wiki a preview of work in progress rather than the accepted documentation state.

**Non-goals.**
- Checking out, resetting, or otherwise mutating the deployment working tree to publish. Publication performs exactly one network operation, `git fetch --prune`, and needs only a read-only deployment credential.
- Publishing an arbitrary active task branch. Only `main`, `master`, `develop`, `release/*`, a release tag, or a pinned 40-character commit SHA are publishable, and the accepted set is deliberately configured rather than inferred.
- Making a hosted read session writable. A published Wiki is branch-backed and therefore read-only; editing stays governed by the documented branch flow.
- Rendering a second, statically generated Wiki site. There is one read model, and publication only changes which commit it reads.
- Anonymous publication. The hosted origin stays authenticated like every other route.
- Treating rollback as "promote the older SHA again". Rollback restores a retained snapshot and pins the project, so the next scheduled tick cannot undo the operator's decision.
