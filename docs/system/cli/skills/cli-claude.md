...
| `tool_progress` | Per-tool-call progress heartbeat (CAR 0.7.0 emits it frequently while a tool call is in flight; carries `toolUseId` and an `occurrence` counter). | Nothing — known and ignorable. It does not reach the activity stream and is not counted as a protocol novelty. | `CliProtocolNoveltyTracker` lists it in the Claude known-type set on purpose: there is no per-tool progress UI yet, so it is classified as known/ignorable rather than left to fall through to the unknown-frame path. Revisit once product ships per-tool progress. |
