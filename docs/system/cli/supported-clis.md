...
**Remote rebase-only continuation.** A fenced coding run records its captured session ID and provider usage on completion. The next `source-needs-rebase` claim may resume it once only after the Agent Host verifies the same task, provider, clean-context marker, repository, worktree path and branch SHA. The resumed turn receives only the current integration base, conflict paths, steer, verification plan and terminal protocol. Missing or stale sessions and any lineage mismatch are typed fresh-run decisions. A failed resumed turn starts one fresh policy-qualified fallback; its receipt retains separate resumed-turn usage. The ceiling is 1,211,213 observed tokens and 300 seconds. Provider summaries that arrive only at process exit cannot be preempted at an exact token boundary.
