...
| Claude frames | Known top-level types are `system`, `assistant`, `user`, `result`, `rate_limit_event`, and `tool_progress`. `tool_progress` is a per-tool-call progress heartbeat that CAR 0.7.0 emits frequently; it is classified as known/ignorable, not novel, because there is no per-tool progress UI yet. | Any other structured type produces a novelty event even if the run later completes. |
...
|---|---|---|---|
| Claude Code 2.1.202 | `system/init`, `assistant`, `user`, `result`, informational `rate_limit_event`, and ignorable `tool_progress` (CAR 0.7.0 per-tool-call progress heartbeat); the rate-limit payload occurs with camel-case and snake-case fields. | `result` supplies completion or provider failure. Informational rate-limit frames do not override a later successful terminal. `tool_progress` heartbeats are known/ignorable and never contribute to `totalUnknownFrames`. | P1 Done, P2 NoOp, P3 Blocked, P4 NeedsInput, P5 provider completion without sentinel, P9 crash, P22 rate-limit casing, P23 unknown frame. Plaintext P1 and P5 preserve the former launch form. |
