...
The operator direction on AGT-2839, 18 September 2026, established the target:

> Das Zielbild sollte sein, dass der Remote-Host auch merged. Der lokale Studio soll nur schauen.

The remote host also merges; the local Studio only watches. On 25 September 2026,
the operator adopted the Dossier recommendations D1-D14 for eight implementation
cards. These choices cover gate and integration authority, attempt shape,
verification identity, Windows coverage, recovery, observer behavior, migration
and transitional work. No human sight review or product implementation is claimed.
...
The 18 September inspection used local develop at
`704ed89b87d03e9a85e22019fc12b9b5da196e68`. On 25 September, worktree HEAD
and local develop both equal `09e189092396700adf0cbafbb38e6ad5b89a818d`;
the cited symbols were rechecked against that snapshot. This is repository
evidence, not proof of the installed Windows binary.
...
- [IntegrationAgentRoundService.cs](../../../backend/Features/Pipeline/IntegrationAgentRoundService.cs),
  ...
  provides a current default of two automatic attribution-ambiguity rounds per
  immutable delivery chain. It does not establish an automatic loop for every merge conflict.
...
Git ownership and a new review for changed content. Adopt one automatic
...
Apply adopted D1-D14, implement durable authority and executor, then shadow one
project for N=20 consecutive comparable cards. Local integration remains the
...
12-hour Studio-off window and stale-writer rejection. N and duration were
adopted on 25 September; they are acceptance minima, not reliability proof.
...
- **2026-08-18 (AGT-2671):** GateSubject/GateAttempt/GateLease model extracted
  from the gate-only Dossier. D1-D6 originally open; no implementation at that date.
...
- **2026-09-18 (AGT-2881):** Operator direction extends ownership through merge
  and push. D7-D14 proposed at that date. Develop inspection corrects historical SSH claims,
...
No cards created. Adopted, implementation-ready prompts are in
...
- **2026-09-25 (AGT-2881 continuation):** The operator adopted the recommended
  D1-D14 options for implementation planning. The Dossier now contains eight
  ordered cards with bounded acceptance, task type and dependencies. The
  staged-page location preserves the one-directory concept boundary; the
  canonical URL awaits the separately scoped publication card.
