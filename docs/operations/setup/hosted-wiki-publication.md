...
This page owns the operational contract. The read model it publishes into is
...
new one. A reader that arrives between the swap and the cache rebuild sees a
complete previous-generation tree, never a missing or mixed one. Tree, page,
