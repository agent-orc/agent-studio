...
in the source hash. Authority rows whose task folder or required parent record
is absent are preserved in `legacy_migration_orphans` with an
`orphaned_task_key` marker instead of aborting the import. Inventory and signed
report fields count orphaned coding attempts, review attempts, leases, fence
counters, and integration records separately; warnings name every non-zero
kind. These rows are evidence-only and never become claimable authority. Bus
logs use a different policy: each JSONL file is counted, hashed, sized, and
stored as a pointer to the frozen root, but its messages are not replayed into
the Task Server event stream.
