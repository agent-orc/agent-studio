...
in the source hash. Bus logs use a different policy: each JSONL file is counted,
hashed, sized, and stored as a pointer to the frozen root, but its messages are
not replayed into the Task Server event stream.
