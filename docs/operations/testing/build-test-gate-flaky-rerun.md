...
for the shell and descendants, mean and peak host CPU, host processor count, mean external CPU, sample availability,
and time under external contention. CPU is a sampled lower bound: a child that
starts and exits between snapshots may be missed. Unsupported or unavailable
telemetry does not authorize an extension. Linux host CPU uses the host's CPU
count, not a container quota's processor count.
...
normal console verbosity. Each command has a unique temporary results folder;
TRX files are parsed and removed after extracting evidence. Streaming console
