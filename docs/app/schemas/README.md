...
- `run-record.schema.json` - one run-timeline row, including the business trigger, actor, reason, and source independently from CLI session resume mechanics. Legacy rows omit trigger provenance and render as not recorded.
...
- `project-execution.schema.json` - repository-owned `.agent-studio/project.yml` preparation, verification, and Stable release identity composition read at the subject commit.
