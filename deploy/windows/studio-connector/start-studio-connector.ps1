...
param(
    ...
    [string] $StateDirectory = (Join-Path $env:ProgramData 'AgentOrchestrator\studio-connector'),
...
$mutexName = 'Local\AgentOrchestratorStudioConnectorStart'
...
try {
    ...
    if (-not (Test-Path -LiteralPath $executablePath)) {
        throw "Studio connector executable not found: $executablePath"
    }
    ...
    while ($true) {
        ...
        $stdoutPath = Join-Path $StateDirectory "studio-connector-$attemptId.stdout.log"
        $stderrPath = Join-Path $StateDirectory "studio-connector-$attemptId.stderr.log"
