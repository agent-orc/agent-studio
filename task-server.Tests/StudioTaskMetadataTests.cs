...
public sealed class StudioTaskMetadataTests
{
    ...
    public async Task Tags_replaces_the_set_validates_existence_and_rejects_a_stale_expected_version()
    {
        ...
        await InsertTagAsync(store, "tag-1", project.ProjectId, "urgent");
        await InsertTagAsync(store, "tag-2", project.ProjectId, "frontend");
        var response = await client.PutAsJsonAsync(
    ...
    private static async Task InsertEpicAsync(TaskServerStore store, string epicId, string projectId, string name)
    {
        ...
        command.CommandText = """
            INSERT INTO epics(id, project_id, name, created_at, updated_at)
            VALUES ($id, $project, $name, $now, $now);
            """;
        command.Parameters.AddWithValue("$id", epicId);
    ...
    private static async Task InsertTagAsync(TaskServerStore store, string tagId, string projectId, string name)
    {
        await using var connection = new SqliteConnection($"Data Source={store.DatabasePath}");
        await connection.OpenAsync();
        await using var command = connection.CreateCommand();
