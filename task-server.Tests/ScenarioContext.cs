...
public sealed class ScenarioContext : IDisposable
{
    ...
    private bool _usesExternalTopology;
    private string _externalWorkRoot = "";
    private string _restoreServerUrl = "";
    private string _activeRunnerId = CodingRunnerId;
    ...
    public static async Task<ScenarioContext> CreateAsync(string root, ScenarioFixture fixture)
    {
        ...
        var externalUrl = Environment.GetEnvironmentVariable("SCENARIO_EXTERNAL_SERVER_URL");
        if (string.IsNullOrWhiteSpace(externalUrl))
            await context.StartServerAsync();
        else
            await context.ConnectToExternalTopologyAsync(externalUrl);
        return context;
    }
    ...
    private async Task ConnectToExternalTopologyAsync(string serverUrl)
    {
        _usesExternalTopology = true;
        _externalWorkRoot = Environment.GetEnvironmentVariable("SCENARIO_EXTERNAL_WORK_ROOT")
            ?? throw new InvalidOperationException("SCENARIO_EXTERNAL_WORK_ROOT is required for an external scenario topology.");
        _restoreServerUrl = Environment.GetEnvironmentVariable("SCENARIO_EXTERNAL_RESTORE_URL")
            ?? throw new InvalidOperationException("SCENARIO_EXTERNAL_RESTORE_URL is required for an external scenario topology.");
        _activeRunnerId = Environment.GetEnvironmentVariable("SCENARIO_EXTERNAL_RUNNER_ID")
            ?? "scenario-compose-runner";
        Directory.CreateDirectory(_externalWorkRoot);
        var repository = Path.Combine(_externalWorkRoot, "repository");
        Directory.CreateDirectory(repository);
        _bareRepositoryPath = await SeedRepositoryAsync(repository);
        _fakeCliReleaseFile = Path.Combine(_externalWorkRoot, "release");
        _dataDirectory = Path.Combine(_externalWorkRoot, "source-store");
        _serverUrl = serverUrl.TrimEnd('/');
        _serverClient = ProtocolClient(
            _serverUrl,
            Environment.GetEnvironmentVariable("SCENARIO_EXTERNAL_AUTH_TOKEN"));
        _disposables.Add(_serverClient);
    }
    ...
    {
        if (_usesExternalTopology)
        {
            await WaitForConditionAsync(
                async () =>
                {
                    var runners = await _serverClient.GetFromJsonAsync<List<RunnerDto>>("/api/v1/runners");
                    return runners?.Any(runner => runner.RunnerId == _activeRunnerId) == true;
                },
                process: null,
                TimeSpan.FromSeconds(30),
                $"runner '{_activeRunnerId}' registered");
            return $"container runner '{_activeRunnerId}' is registered";
        }
        ...
        _runner = StartBuilt(
            ...
            "--poll",
            "--server", _serverUrl,
            "--runner-id", _activeRunnerId,
            "--runner-name", _activeRunnerId,
        ...
        await WaitForConditionAsync(
            ...
            $"runner '{_activeRunnerId}' registered");
        return $"runner '{_activeRunnerId}' is registered";
    }
    ...
    private async Task<string?> RestoreIntoEmptyStoreAsync()
    {
        if (_usesExternalTopology)
        {
            var externalTargetData = Path.Combine(_externalWorkRoot, "restore-store");
            using var externalTargetClient = ProtocolClient(
                _restoreServerUrl,
                Environment.GetEnvironmentVariable("SCENARIO_EXTERNAL_AUTH_TOKEN"));
            return await RestoreIntoStoreAsync(externalTargetData, externalTargetClient);
        }
        ...
        return await RestoreIntoStoreAsync(targetData, targetClient);
    }

    private async Task<string?> RestoreIntoStoreAsync(string targetData, HttpClient targetClient)
    {
        ...
        File.Copy(
            _backupResult.Path,
            Path.Combine(backupsDirectory, Path.GetFileName(_backupResult.Path)),
            overwrite: true);
    ...
    private Task<string> CreateFakeCodingCliAsync(string directory)
    {
        var path = Path.Combine(directory, "scenario-coding-agent.sh");
        File.Copy(
            Path.Combine(_root, "testsupport", "scenario", "fake-coding-cli.sh"),
            path,
            overwrite: true);
