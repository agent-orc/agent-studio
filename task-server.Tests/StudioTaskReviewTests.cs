...
/// <para>
/// <c>Program.cs</c> is off-limits to this group (see the group's
/// ownership rules), so the real app does not yet call
/// <c>MapStudioTaskReviewEndpoints</c> or
/// <c>ApplyStudioTaskReviewMigrationAsync</c> -- that wiring is a separate,
/// later integration step. Each test therefore (1) applies this group's
/// migration directly against the factory's store before exercising any
/// route, and (2) registers this group's endpoints into the test host via
/// an <see cref="IStartupFilter"/> added through
/// <c>WebApplicationFactory.WithWebHostBuilder</c>, without modifying
/// <c>StudioTestApiFactory</c> or <c>Program.cs</c>.
/// </para>
/// </summary>
public sealed class StudioTaskReviewTests
{
    ...
    /// <summary>
    /// Builds a <see cref="StudioTestApiFactory"/>-based host with this
    /// group's endpoints mapped (via an <see cref="IStartupFilter"/>, since
    /// <c>Program.cs</c> cannot be edited by this group) and this group's
    /// migration applied (since <c>TaskServerStore.cs</c>'s migration
    /// pipeline cannot be edited by this group either). Both are wired for
    /// real once the coordinating integration pass lands them in the shared
    /// files.
    /// </summary>
    private static async Task<WebApplicationFactory<Program>> CreateReadyFactoryAsync(string dataDirectory)
    {
        var factory = new StudioTestApiFactory(dataDirectory).WithWebHostBuilder(builder =>
            builder.ConfigureServices(services =>
                services.AddSingleton<IStartupFilter, MapStudioTaskReviewEndpointsStartupFilter>()));
        var store = factory.Services.GetRequiredService<TaskServerStore>();
        await using var connection = new SqliteConnection($"Data Source={store.DatabasePath};Pooling=False");
        await connection.OpenAsync();
        await store.ApplyStudioTaskReviewMigrationAsync(connection, default);
        return factory;
    }
    ...
    /// <summary>
    /// Registers this group's routes into the WebApplicationFactory-hosted
    /// <c>Program</c> app without editing <c>Program.cs</c>. Calling
    /// <c>next(app)</c> first runs the app's own configuration (including its
    /// implicit <c>UseRouting()</c>, since <c>Program.cs</c> never calls it
    /// explicitly), which populates the ambient endpoint route builder that
    /// <c>UseEndpoints</c> then reuses -- so this appends to the exact same
    /// live endpoint data source the real app's own <c>app.MapTaskServerEndpoints()</c>
    /// / <c>app.MapStudioEndpoints()</c> calls populate.
    /// </summary>
    private sealed class MapStudioTaskReviewEndpointsStartupFilter : IStartupFilter
    {
        public Action<IApplicationBuilder> Configure(Action<IApplicationBuilder> next)
            => app =>
            {
                next(app);
                app.UseEndpoints(endpoints => endpoints.MapStudioTaskReviewEndpoints());
            };
    }
}
