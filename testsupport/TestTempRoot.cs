...
/// per-process directory under <c>&lt;host temp&gt;/agent-studio-tests</c>.
/// Every fixture that keeps calling <c>Path.GetTempPath()</c> therefore lands
/// inside that directory, and the whole directory is removed when the process
/// exits. A killed process leaves exactly one entry instead of a hundred, and
/// the next process to start sweeps it away once it is older than
...
{
    /// <summary>Single directory the per-process suite roots live under.</summary>
    public const string SuiteRootPrefix = "agent-studio-tests";
    ...
    /// temp path so not even the suite root touches the shared temp root.
    /// </summary>
    ...
    public static string Redirect()
    {
        lock (Gate)
        {
            ...
            var baseDirectory = string.IsNullOrWhiteSpace(configured)
                ? Path.Combine(host, SuiteRootPrefix)
                : Path.GetFullPath(configured.Trim());
            Directory.CreateDirectory(baseDirectory);
            SweepStaleSuiteRoots(baseDirectory, DateTime.UtcNow);
    ...
    public static string Create(string prefix)
    {
        ...
        var path = Path.Combine(root, $"{prefix.TrimEnd('-')}-{Guid.NewGuid():N}");
