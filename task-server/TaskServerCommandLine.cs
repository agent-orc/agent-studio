    ...
    string[] HostArguments)
{
    public static TaskServerCommandLine Parse(string[] args)
    {
        if (args is ["--version"] or ["-V"])
            return new TaskServerCommandLine(TaskServerCommandKind.Version, null, null, null, null, []);
        if (args.Length == 0 || !IsCommand(args[0]))
            return new TaskServerCommandLine(TaskServerCommandKind.Serve, null, null, null, null, args);
