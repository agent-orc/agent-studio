...
public sealed partial class TaskServerStore
{
    ...
    private async Task<ProviderModelFallback?> ReadProviderFallbackForClaimAsync(
        SqliteConnection connection,
        SqliteTransaction transaction,
        TaskDto task,
        CancellationToken ct)
    {
        ...
        await using (var reader = await command.ExecuteReaderAsync(ct))
            if (await reader.ReadAsync(ct)) payload = reader.GetString(0);
        ...
        var synthetic = new CompleteRunRequest(
            ...
            SalvageBranch: decision.RawFacts.DurableOutputReference);
        return (await PlanProviderRejectionAsync(
