...
public sealed class LegacyMigrationService(TaskServerStore store)
{
        ...
        CancellationToken ct)
    {
        ...
        return ResultFromReport(report, idempotent: false);
    }
    ...
    private static LegacyMigrationInventory BuildInventory(string root, LegacyScan scan)
    {
        ...
        var draft = new LegacyMigrationInventory(
            ...
            OrphanedCodingAttempts: scan.Orphans.Count(item => item.Kind == "coding-attempt"),
            OrphanedReviewAttempts: scan.Orphans.Count(item => item.Kind == "review-attempt"),
            OrphanedLeases: scan.Orphans.Count(item => item.Kind == "lease"),
            OrphanedFenceCounters: scan.Orphans.Count(item => item.Kind == "fence-counter"),
            OrphanedIntegrationRecords: scan.Orphans.Count(item => item.Kind == "integration-record"),
    ...
    private static string ComputeInventorySha256(LegacyMigrationInventory inventory)
    {
        var canonical = new
        {
            ...
            inventory.OrphanedCodingAttempts,
            inventory.OrphanedReviewAttempts,
            inventory.OrphanedLeases,
            inventory.OrphanedFenceCounters,
            inventory.OrphanedIntegrationRecords,
        ...
        CancellationToken ct)
    {
        ...
        var unsigned = new LegacyMigrationReport(
            ...
            "Closed coding and review history is imported. Open leases are retained as process-unknown with their fences.",
    ...
    private static LegacyMigrationResult ResultFromReport(LegacyMigrationReport report, bool idempotent)
        => new(
            report.MigrationId,
            true,
    ...
    private static async Task<LegacyScan> ScanAsync(string root, bool includeContent, CancellationToken ct)
    {
        ...
        var orphans = FindOrphans(projects, authority, supplementary);
        foreach (var group in orphans.GroupBy(item => item.Kind, StringComparer.Ordinal).OrderBy(item => item.Key, StringComparer.Ordinal))
            warnings.Add(
                $"Preserved {group.Count()} orphaned {group.Key} record(s) in the legacy migration ledger; " +
                "their task folders or required parent records are absent.");
        ...
        CancellationToken ct)
    {
        ...
        foreach (var dossierRoot in dossierRoots.Where(Directory.Exists))
        foreach (var path in Directory.EnumerateFiles(dossierRoot, "workbench.json", SearchOption.AllDirectories).Order(StringComparer.Ordinal))
        {
            ct.ThrowIfCancellationRequested();
            sourceFiles.Add(path);
    ...
    private static IReadOnlyList<LegacyOrphanImport> FindOrphans(
        ...
        IReadOnlyList<LegacySupplementaryImport> supplementary)
    {
        ...
        var codingAttemptIds = authority.CodingAttempts
            .Select(attempt => attempt.AttemptId)
            .ToHashSet(StringComparer.Ordinal);
        var orphanedCodingAttemptIds = authority.CodingAttempts
            .Where(attempt => !taskKeys.Contains(attempt.TaskKey))
            .Select(attempt => attempt.AttemptId)
            .ToHashSet(StringComparer.Ordinal);
        var result = new List<LegacyOrphanImport>();

        foreach (var attempt in authority.CodingAttempts.Where(attempt => orphanedCodingAttemptIds.Contains(attempt.AttemptId)))
        {
            result.Add(new LegacyOrphanImport(
                "coding-attempt", attempt.AttemptId, attempt.TaskKey,
                "task-folder-missing", JsonSerializer.Serialize(attempt, Json)));
            if (attempt.Lease is { } lease)
                result.Add(new LegacyOrphanImport(
                    "lease", lease.LeaseId, attempt.TaskKey,
                    "coding-attempt-task-folder-missing", JsonSerializer.Serialize(lease, Json)));
        }

        foreach (var attempt in authority.ReviewAttempts)
        {
            var reason = !taskKeys.Contains(attempt.TaskKey)
                ? "task-folder-missing"
                : !codingAttemptIds.Contains(attempt.SourceRunAttemptId)
                    ? "source-coding-attempt-missing"
                    : orphanedCodingAttemptIds.Contains(attempt.SourceRunAttemptId)
                        ? "source-coding-attempt-orphaned"
                        : null;
            if (reason is null) continue;
            result.Add(new LegacyOrphanImport(
                "review-attempt", attempt.AttemptId, attempt.TaskKey,
                reason, JsonSerializer.Serialize(attempt, Json)));
            if (attempt.Lease is { } lease)
                result.Add(new LegacyOrphanImport(
                    "lease", lease.LeaseId, attempt.TaskKey,
                    $"review-attempt-{reason}", JsonSerializer.Serialize(lease, Json)));
        }

        foreach (var (taskKey, fence) in authority.LastFenceByTask.Where(item => !taskKeys.Contains(item.Key)))
            result.Add(new LegacyOrphanImport(
                "fence-counter", taskKey, taskKey,
                "task-folder-missing", JsonSerializer.Serialize(new { taskKey, fence }, Json)));

        foreach (var entity in supplementary.Where(item => item.Kind == "integration-record"))
        {
            using var document = JsonDocument.Parse(entity.PayloadJson);
            var payload = entity.PayloadJson;
            var taskKey = ReadString(document.RootElement, "key") ?? ReadString(document.RootElement, "taskKey");
            if (document.RootElement.TryGetProperty("integrationRecords", out var records)
                && records.ValueKind == JsonValueKind.Array
                && int.TryParse(entity.Key.Split(':').LastOrDefault(), out var index)
                && index >= 0
                && index < records.GetArrayLength())
            {
                var record = records[index];
                taskKey = ReadString(record, "orphanedTaskKey") ?? ReadString(record, "taskKey") ?? taskKey;
                payload = record.GetRawText();
            }
            if (taskKey is null || taskKeys.Contains(taskKey)) continue;
            result.Add(new LegacyOrphanImport(
                "integration-record", entity.Key, taskKey.Trim().ToUpperInvariant(),
                "task-folder-missing", payload));
        }
        return result;
    }
