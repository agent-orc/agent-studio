...
/// (tasks, events, leases, audit, review attempts) are never enumerated as payload, so the planner and executor
/// can never prune or move their authority content. Coordination still updates the task archive marker and
/// appends audit rows.
/// </summary>
public sealed class SqliteRetentionStore(TaskServerStore store) : IRetentionStore
{
    ...
    public Task<ArchiveTransition?> MoveToColdAsync(RetentionAction action, RetentionPolicy policy, CancellationToken cancellationToken = default)
        => store.MoveRetentionActionToColdAsync(action, policy, cancellationToken);
    ...
    public Task DeleteColdAsync(RetentionAction action, CancellationToken cancellationToken = default)
        => store.DeleteColdRetentionPayloadsAsync(action, cancellationToken);
...
public sealed record RetentionManifestEnvelope(
    ...
    DateTimeOffset? TombstonedAt = null);
...
public sealed partial class TaskServerStore
{
    ...
    internal async Task<IReadOnlyList<RetentionTaskInventory>> EnumerateRetentionInventoryAsync(CancellationToken ct)
    {
        ...
        await using var command = Command(connection, """
            SELECT t.id, t.task_key, p.id, t.state, t.updated_at,
                   CASE WHEN a.run_id = '' THEN 'legacy' ELSE a.run_id END,
                   a.id, a.name, a.size_bytes, a.created_at, a.archived, am.state
              FROM tasks t
              JOIN projects p ON p.id = t.project_id
              LEFT JOIN artifacts a
                ON a.run_id IN (SELECT r.id FROM runs r WHERE r.task_id = t.id)
                OR (a.run_id = '' AND a.idempotency_key LIKE 'legacy:' || t.id || ':artifact:%')
              LEFT JOIN archive_manifests am ON am.task_id = t.id
             ORDER BY t.id;
            """);
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            if (reader.GetInt64(10) != 0
                && !reader.IsDBNull(11)
                && string.Equals(reader.GetString(11), "tombstone", StringComparison.OrdinalIgnoreCase))
                continue;
            var key = RetentionFileKey(reader.GetString(5), reader.GetString(6), reader.GetString(7));
            entry.Files.Add(new RetentionFile(
                key,
                reader.GetInt64(8),
                new DateTimeOffset(Parse(reader.GetString(9))),
                classifier.Classify(reader.GetString(7)),
    ...
    internal async Task<ArchiveTransition?> MoveRetentionActionToColdAsync(
        RetentionAction action,
        RetentionPolicy policy,
        CancellationToken ct)
    {
        if (action.Files.Count == 0)
            return action.Kind == RetentionActionKind.ArchiveTask
                ? await MarkWholeTaskArchivedWithoutNewPayloadAsync(action, policy, ct)
                : null;
    ...
    internal async Task RestoreRetentionTaskAsync(string taskKey, CancellationToken ct)
    {
        ...
        await using (var connection = await OpenReadyAsync(ct))
        {
            ...
            if (envelope.TombstonedAt is not null)
                throw new InvalidOperationException($"Task '{taskKey}' has a tombstone manifest and no cold payload.");
        }

        await SetArchiveManifestStateAsync(taskId, "restoring", ct);
        try
        {
        ...
        {
            await SetArchiveManifestStateAsync(taskId, "cold", ct);
            throw;
        }
    }

    internal async Task DeleteColdRetentionPayloadsAsync(RetentionAction action, CancellationToken ct)
    {
        RetentionManifestEnvelope envelope;
        await using (var connection = await OpenReadyAsync(ct))
            envelope = await ReadManifestEnvelopeAsync(connection, null, action.Task.StoreKey, ct)
                ?? throw new InvalidOperationException($"Task '{action.Task.TaskKey}' is not archived.");
        if (envelope.TombstonedAt is not null)
            return;
        ...
        try
        {
            ...
            foreach (var (_, pendingPath) in pending)
                File.Delete(pendingPath);
        }
        catch
        {
            if (!committed)
                foreach (var (original, pendingPath) in pending.AsEnumerable().Reverse())
                    if (File.Exists(pendingPath) && !File.Exists(original))
                        File.Move(pendingPath, original);
            throw;
        }
    }

    private async Task SetArchiveManifestStateAsync(string taskId, string state, CancellationToken ct)
        => await InWriteTransactionAsync(async (connection, transaction)
    ...
    private async Task<ArchiveTransition> MarkWholeTaskArchivedWithoutNewPayloadAsync(
        ...
        CancellationToken ct)
    {
        var archivedAt = new DateTimeOffset(UtcNow);
        await InWriteTransactionAsync(async (connection, transaction) =>
        {
            var existing = await ReadManifestEnvelopeAsync(connection, transaction, action.Task.StoreKey, ct)
                           ?? throw new InvalidOperationException(
                               $"Task '{action.Task.TaskKey}' has no cold payload to promote to archive stage 2.");
            var stage = new RetentionManifestStage(
                2, archivedAt, string.Empty, string.Empty, 0, [], policy.Version, policy.UpdatedBy);
            var envelope = existing with
            {
                Stages = existing.Stages.Where(item => item.Stage != 2).Append(stage).OrderBy(item => item.Stage).ToList(),
            };
            await ExecuteAsync(connection, """
                UPDATE archive_manifests
                   SET manifest_json = $manifest,
                 ...
                 WHERE task_id = $task;
                UPDATE tasks
                   SET archive_state = 'archived', archived_at = $at
                 WHERE id = $task;
                """, ct, transaction,
                ("$manifest", JsonSerializer.Serialize(envelope, RetentionJson)),
                ("$task", action.Task.StoreKey),
                ("$at", Iso(archivedAt.UtcDateTime)));
        }, ct);
        return new ArchiveTransition(string.Empty, string.Empty, archivedAt, 2, 0);
    }
