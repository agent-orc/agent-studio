...
/// never touch them.
/// </summary>
public sealed class SqliteRetentionStore(TaskServerStore store, string actorId = "retention-engine") : IRetentionStore
{
    ...
    public Task<ArchiveTransition?> MoveToColdAsync(RetentionAction action, RetentionPolicy policy, CancellationToken cancellationToken = default)
        => store.MoveRetentionActionToColdAsync(action, policy, actorId, cancellationToken);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(
                manifestPath,
                JsonSerializer.Serialize(manifest, RetentionJson),
                Encoding.UTF8,
                ct);
            ...
            return new ArchiveTransition(manifestPath, payloadPath, archivedAt, action.Stage, totalBytes);
        }
        catch
        {
            // Once the database manifest is committed, the cold copy is authoritative. Leaving both copies
            // after a later hot-clear failure is safe and permits a retry; deleting cold data would not be.
            if (!manifestCommitted && Directory.Exists(directory)) Directory.Delete(directory, recursive: true);
            throw;
        }
    }
