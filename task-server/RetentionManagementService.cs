...
public sealed class RetentionManagementService(TaskServerStore store)
{
    ...
    internal Task<RetentionApplyResultDto> ApplyScheduledAsync(string actorId, CancellationToken ct)
        => store.ApplyScheduledRetentionRunAsync(actorId, ct);

    internal Task<bool> LoadGateAllowsAsync(CancellationToken ct)
        => store.RetentionLoadGateAllowsAsync(ct);
...
public sealed partial class TaskServerStore
{
        ...
        SqliteConnection connection, SqliteTransaction? transaction, CancellationToken ct)
    {
        ...
        await using var command = Command(connection, """
            SELECT retention_policies.policy_json, projects.name
              FROM retention_policies
              JOIN projects ON projects.id = substr(retention_policies.scope, length('project:') + 1)
             WHERE retention_policies.scope LIKE 'project:%';
            """, transaction);
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            overrides[reader.GetString(1)] = new ProjectRetentionOverride { Rules = rules };
        }
        return overrides;
    }
    ...
    internal async Task<RetentionApplyResultDto> ApplyRetentionRunAsync(RunRetentionRequest request, string actorId, CancellationToken ct)
        => await ApplyRetentionRunCoreAsync(request, "manual", actorId, ct);

    internal async Task<RetentionApplyResultDto> ApplyScheduledRetentionRunAsync(string actorId, CancellationToken ct)
        => await ApplyRetentionRunCoreAsync(new RunRetentionRequest(), "scheduled", actorId, ct);
        ...
        CancellationToken ct)
    {
        ...
        var plan = new RetentionPlanner().Plan(inventory, policy, UtcNow);
        var result = await new RetentionExecutor(new SqliteRetentionStore(this, actorId)).ApplyAsync(plan, policy, ct);
        var dto = ToRetentionPlanDto(plan);
    ...
    internal async Task<bool> RetentionLoadGateAllowsAsync(CancellationToken ct)
    {
        var freshAfter = UtcNow.AddMinutes(-5);
        foreach (var snapshot in await ListRunnerCapabilitySnapshotsAsync(ct))
        {
            var telemetry = snapshot.Telemetry;
            if (telemetry is null || telemetry.ObservedAt < freshAfter)
                continue;
            var loadPercent = telemetry.CpuPercent
                              ?? (telemetry.Load1.HasValue && telemetry.CpuCores > 0
                                  ? telemetry.Load1.Value / telemetry.CpuCores * 100d
                                  : null);
            var target = snapshot.RuntimeCapacity?.TargetLoadPercent ?? 80;
            if (loadPercent >= target)
                return false;
        }
        return true;
    }
    ...
    {
        RequireWritable();
        if (requestedStage is not (null or 1 or 2))
            throw new ArgumentException("Archive stage must be 1 or 2.");
        var policy = await GetActiveRetentionPolicyAsync(ct);
        var inventory = await EnumerateRetentionInventoryAsync(ct);
        ...
        if (task.TerminalAt is null || heavyRule.NeverArchiveLanes.Contains(task.Lane))
            throw new TaskServerConflictException(
                "retention-task-active",
                $"Task '{task.TaskKey}' is in active lane '{task.Lane}' and cannot be archived.");
        if ((await GetTaskIdsWithActiveLeasesAsync(ct)).Contains(task.StoreKey))
            throw new TaskServerConflictException(
                "retention-task-leased",
                $"Task '{task.TaskKey}' has an active lease and cannot be archived.");
        ...
        var stageTwo = RetentionPlanner.SelectStageTwoFiles(task.Files);
        var action = stage >= 2
