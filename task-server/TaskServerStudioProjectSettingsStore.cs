...
public sealed partial class TaskServerStore
{
    public async Task<StudioProjectCliModesResponse> GetProjectCliModesAsync(string projectIdentity, CancellationToken ct)
    {
        ...
        var resolved = CliTypes.All.ToDictionary(
            cli => cli,
            cli => new StudioCliModeResolution(CliPermissionModes.Yolo, "default", CliPermissionFlags.For(cli, CliPermissionModes.Yolo)),
            StringComparer.OrdinalIgnoreCase);
        return new StudioProjectCliModesResponse(resolved, new Dictionary<string, string>(StringComparer.Ordinal), CliPermissionModes.UserVisible);
    }
    ...
    public async Task<StudioProjectCliContextModesResponse> GetProjectCliContextModesAsync(string projectIdentity, CancellationToken ct)
    {
        ...
        var resolved = CliTypes.All.ToDictionary(
            cli => cli,
            cli => new StudioCliContextModeResolution(CliContextModes.Clean, "default", CliContextModes.SupportsClean(cli)),
            StringComparer.OrdinalIgnoreCase);
        return new StudioProjectCliContextModesResponse(resolved, new Dictionary<string, string>(StringComparer.Ordinal), CliContextModes.UserVisible);
    }
    ...
    public async Task<IReadOnlyDictionary<string, StudioProjectSettingsEntry>> GetAllProjectSettingsAsync(CancellationToken ct)
    {
        ...
        var cliDefaults = CliTypes.All.ToDictionary(
            cli => cli,
            cli => new StudioCliModeResolution(CliPermissionModes.Yolo, "default", CliPermissionFlags.For(cli, CliPermissionModes.Yolo)),
            StringComparer.OrdinalIgnoreCase);
