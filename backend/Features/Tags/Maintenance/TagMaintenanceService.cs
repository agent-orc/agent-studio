            ...
            try
            {
                foreach (var change in decision.Changes)
                {
                    if (workspace.Write(change))
                    {
                        ...
                        try
                        {
                            if (workspace.Read(change) != change.After)
                                throw new InvalidOperationException("The persisted value does not match the approved change.");
                        }
                        catch (Exception ex)
                        {
                            var detail = $"{change.Kind}/{change.Id}: mutation applied, verification failed: {ex.Message}";
                            state.Audit.Add(new(time.GetUtcNow(), id, actor, "verification-failed", detail));
                            Save(project, state);
