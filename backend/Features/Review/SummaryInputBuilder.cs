...
public static class SummaryInputBuilder
{
    ...
    public static SummaryInputs Build(TaskInfo info, string rawLog, GitService? git = null)
    {
        ...
        return new SummaryInputs(
            ...
            Delivery: TruncateTail(BuildDelivery(info, pipeline, git), DeliveryBudget),
    ...
    private static string BuildDelivery(
        TaskInfo info,
        PipelineExecutionRecord? pipeline,
        GitService? git)
    {
        ...
        AppendDeliveryDiffStat(sb, info, activeCommits, git);
    ...
    private static void AppendDeliveryDiffStat(
        StringBuilder sb,
        TaskInfo info,
        IReadOnlyList<TaskCommitInfo> activeCommits,
        GitService? git)
    {
        var repoRoot = git?.ResolveRepoRoot(info.Id, info.WatchPath)
            ?? git?.ResolveProjectRepoRoot(info.ProjectName);
        if (git is null || string.IsNullOrWhiteSpace(repoRoot) || !Directory.Exists(repoRoot))
        {
            AppendMetadataDiffFallback(sb, activeCommits);
            return;
        }

        var reviewSubject = ReviewSubjectStore.Read(info.FolderPath);
        var lastCommit = activeCommits.LastOrDefault();
        var deliveryRef = ReviewSubjectStore.IsValidResultSha(reviewSubject?.ResultSha)
            ? reviewSubject!.ResultSha
            : !string.IsNullOrWhiteSpace(lastCommit?.ResultSha)
                ? lastCommit.ResultSha
                : lastCommit?.Sha;
        if (string.IsNullOrWhiteSpace(deliveryRef))
        {
            sb.AppendLine("git diff --stat against merge base: unavailable (no delivery revision recorded).");
            return;
        }
        var integrationBranch = TaskIntegrationBranch.Name(info.IntegrationBranch, fallback: string.Empty);
        var recordedMergeBase = ReviewVerificationStore.Read(info.FolderPath)?.MergeBaseSha
            ?? reviewSubject?.BaseSha
            ?? info.Provenance?.Base;
        var result = git.GetDiffStatAgainstMergeBase(
            repoRoot,
            integrationBranch,
            deliveryRef,
            recordedMergeBase,
            maxChars: DeliveryBudget / 2);
        if (!result.Success)
        {
            var error = CredentialRedactor.Redact(result.Error ?? "unknown Git error");
            if (error.Length > 400) error = error[..400] + "...";
            sb.AppendLine($"git diff --stat against merge base: unavailable ({error}); attributed metadata was not substituted.");
            return;
        }

        var shortBase = result.MergeBaseSha is { Length: > 12 }
            ? result.MergeBaseSha[..12]
            : result.MergeBaseSha;
        sb.AppendLine($"git diff --stat against merge base `{shortBase}` with `{result.IntegrationRef}` (net delivery):");
        sb.AppendLine("```text");
        sb.AppendLine(result.Stat);
        sb.AppendLine("```");
    }

    private static void AppendMetadataDiffFallback(
        StringBuilder sb,
        IReadOnlyList<TaskCommitInfo> activeCommits)
    {
        ...
        sb.AppendLine(
            $"git diff --stat against merge base: {reportedCount} files changed "
            + "(fallback from attributed delivery metadata; repository unavailable).");
