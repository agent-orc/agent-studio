...
public sealed class SummaryGenerationService
{
    ...
    private readonly ConcurrentDictionary<string, SummaryGenerationCoordinator> _coordinators =
    ...
    /// <summary>
    /// Pure projection helper retained for state readers and focused tests.
    /// Scheduling is owned by the per-task coordinator below, so a stale
    /// presentation state can never suppress a new generation. A job is
    /// considered "still generating" when its previous
    ...
    /// <summary>
    /// Guarantees a generation using evidence observed after any currently
    /// running generation. Terminal lane completion uses this because the
    /// integration and gate facts may have arrived after the earlier prompt
    /// was rendered. Concurrent refresh requests coalesce to one follow-up.
    /// </summary>
    public Task RegenerateAfterCurrentAsync(TaskInfo info, CancellationToken ct = default)
        => CoordinateGenerationAsync(info, runOutcome: null, refreshAfterCurrent: true, ct);
    ...
    public Task GenerateAsync(
        TaskInfo info,
        TerminalRunOutcome? runOutcome,
        CancellationToken ct = default)
        => CoordinateGenerationAsync(info, runOutcome, refreshAfterCurrent: false, ct);

    private Task CoordinateGenerationAsync(
        TaskInfo info,
        TerminalRunOutcome? runOutcome,
        bool refreshAfterCurrent,
        CancellationToken ct)
    {
        var key = info.TaskKey;
        var coordinator = _coordinators.GetOrAdd(key, static _ => new SummaryGenerationCoordinator());
        var request = new SummaryGenerationRequest(info, runOutcome, ct);

        lock (coordinator.Sync)
        {
            if (coordinator.Active is { IsCompleted: false } active)
            {
                if (refreshAfterCurrent)
                {
                    coordinator.QueuedRefresh = request;
                    _logger.LogInformation(
                        "Queued Result regeneration after the inflight summary for {JobId}",
                        info.Id);
                }
            ...
            coordinator.Active = completion.Task;
            _ = RunCoordinatedGenerationsAsync(coordinator, request, completion);
            return completion.Task;
        }
    }

    private async Task RunCoordinatedGenerationsAsync(
        SummaryGenerationCoordinator coordinator,
        ...
        TaskCompletionSource completion)
    {
        try
        {
            ...
            while (true)
            {
                await GenerateCoreAsync(current.Info, current.RunOutcome, current.CancellationToken)
                    .ConfigureAwait(false);

                lock (coordinator.Sync)
                {
                    if (coordinator.QueuedRefresh is not null)
                    {
                        current = coordinator.QueuedRefresh;
    ...
    private sealed class SummaryGenerationCoordinator
    {
        public object Sync { get; } = new();
        public Task? Active { get; set; }
        public SummaryGenerationRequest? QueuedRefresh { get; set; }
    }

    private sealed record SummaryGenerationRequest(
        TaskInfo Info,
        TerminalRunOutcome? RunOutcome,
