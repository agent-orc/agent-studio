...
{
    /// <summary>
    /// <c>action</c> value of the receipt an infrastructure or quota replay
    /// writes. Distinct from <c>requeued</c> so the two retry budgets are counted
    /// apart (AGT-2749).
    /// </summary>
    private const string InfrastructureRequeueAction = "requeued-infrastructure";
        ...
        CancellationToken ct)
    {
        ...
        var moved = _scanner.FindJob(job.Id, job.WatchPath);
        if (moved is not null)
            AppendAction(moved, InfrastructureRequeueAction, reason, retryNumber);
        return true;
    }
