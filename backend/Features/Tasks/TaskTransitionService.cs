        ...
        bool archiveOverride = false)
    {
        ...
        MoveJobOutcome MoveCore()
        {
            var moved = _states.MoveJob(
                ...
                transitionDetail,
                archiveOverride);
            if (!archiveOverride || moved.Status != MoveJobStatus.Success)
                return moved;

            // Keep evidence and its rollback inside the terminal lifecycle
            // callback. A failed override must not supersede ReviewAttempts.
            lock (ArchiveOverrideEvidenceLock)
            {
                var archived = _scanner.FindJob(jobId, watchPath)
                    ?? info with { FolderPath = moved.NewFolderPath ?? info.FolderPath };
                ...
                var timelineLength = 0L;
                var auditLength = 0L;
                var evidenceSnapshotTaken = false;
                try
                {
                    timelineLength = File.Exists(timelinePath) ? new FileInfo(timelinePath).Length : 0;
                    auditLength = File.Exists(auditPath) ? new FileInfo(auditPath).Length : 0;
                    evidenceSnapshotTaken = true;
                    if (RecordArchiveOverride(archived, archived.FolderPath, cause!, reason!)
                        && WriteArchiveOverrideAudit(archived, cause!, reason!))
                        return moved;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "archive-override-evidence-failed task={TaskKey}", info.TaskKey);
                }

                if (evidenceSnapshotTaken)
                {
