...
public sealed class TaskTransitionService
{
        ...
        bool archiveOverride = false)
    {
        ...
        if (archiveOverride && outcome.Status == MoveJobStatus.Success)
        {
            ...
            if (WriteArchiveOverrideAudit(archived, cause!, reason!))
            {
                if (!RecordArchiveOverride(archived, archived.FolderPath, cause!, reason!))
                    _logger.LogError("archive-override-timeline-failed task={TaskKey}", info.TaskKey);
            }
            else
                _logger.LogError("archive-override-evidence-failed task={TaskKey}", info.TaskKey);
        }
