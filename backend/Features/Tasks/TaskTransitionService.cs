...
public sealed class TaskTransitionService
{
        ...
        bool archiveOverride = false)
    {
        ...
        if (archiveOverride && !RecordArchiveOverride(info, info.FolderPath, cause!, reason!))
            return new MoveJobOutcome(MoveJobStatus.Failure, "Archive override timeline could not be persisted.");
        if (archiveOverride && !WriteArchiveOverrideAudit(info, cause!, reason!))
            return new MoveJobOutcome(MoveJobStatus.Failure, "Archive override audit could not be persisted.");
        ...
        if (outcome.Status == MoveJobStatus.Success && fromState != targetState)
        {
            ...
            if (boundCompletionClaim is not null)
                RecordCompletionClaim(jobId, watchPath, boundCompletionClaim);
