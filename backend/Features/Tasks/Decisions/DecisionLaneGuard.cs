...
/// <summary>Capability held by the audited decision reopen lifecycle.</summary>
public sealed class DecisionReopenPermit
{
    private DecisionReopenPermit() { }

    internal static DecisionReopenPermit Instance { get; } = new();
    internal bool IsValid => ReferenceEquals(this, Instance);
}
...
public static class DecisionLaneGuard
{
    public static string? Refusal(TaskInfo task, string targetState, TaskReferenceIndex index,
        DecisionReopenPermit? reopenPermit = null)
    {
        if (TaskKinds.IsDecision(task.Kind))
        {
            if (targetState == TaskStates.Preparation)
                return task.Decision?.Status != DecisionStatuses.Decided
                    || reopenPermit?.IsValid == true
                    ? null : "A decided decision card can return to preparation only through decision reopen.";
