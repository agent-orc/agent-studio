...
internal static class OrchestratorConfigCatalog
{
    ...
    public static readonly OrchestratorConfigDefinition[] Definitions =
    {
        ...
        new(
            ...
            "Orchestrator",
            "Global Watcher",
            "Autonomous problem finding. Detects repetition, contradiction, silence, drift, and hygiene across projects and proposes tickets in the proposal state. It never moves a card to Ready and never touches Git.",
            "bool",
            DefaultValue: false,
            SourceFile: "backend/Features/Watcher/WatcherHostedService.cs"),
        new(
            "Watcher:AnalysisEnabled",
            "Orchestrator",
            "Watcher bounded analysis",
            "Allow the Watcher to spend its contingent on a strong analysis call for a case that declared an uncertain cause. Detection never uses a model.",
            "bool",
            DefaultValue: true,
            SourceFile: "backend/Features/Watcher/Analysis/WatcherAnalyst.cs"),
        new(
            "Watcher:IntervalSeconds",
            "Orchestrator",
            "Watcher sweep (seconds)",
            "Reconciliation cadence of the Watcher sweep. The dossier's five-minute default is 300.",
            "int",
            DefaultValue: 300,
            SourceFile: "backend/Features/Watcher/WatcherHostedService.cs"),
        new(
