...
public class ProjectRunner
{
    ...
    private static RunTriggerMetadata TriggerForPendingIntent(PendingIntent intent, TaskInfo info)
    {
        ...
        var actor = trigger switch
        {
            ...
            RunTriggers.OperatorContinue => $"operator {info.OwnerClientId ?? "local-default"}",
            _ => "pipeline",
        };
        ...
        try
        {
            ...
            if (intent == RunIntent.AutoPickup && info.PendingIntent != null)
            {
                ...
                if (stashed != null && !string.IsNullOrWhiteSpace(stashed.Prompt))
                {
                    ...
                    triggerMetadata ??= TriggerForPendingIntent(stashed, info);
                }
            }
                    ...
                    && !string.IsNullOrWhiteSpace(action.FollowupRetryPrompt))
                {
                    ...
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            var retryTrigger = action.IssueKind switch
                            {
                            ...
                            var retryActor = action.IssueKind == RunIssueKind.WatchdogTimeout ? "watchdog" : "pipeline";
                            await RunCliAsync(jobId, RunIntent.UserContinue, retryPrompt, retryAttempt, ContinueModes.Continue, CancellationToken.None,
                                new RunTriggerMetadata(
                                    retryTrigger,
                                    retryActor,
                                    action.MetaMessage,
                                    $"failure={ToIssueTopic(action.IssueKind)}; prompt={RunTriggerMetadata.PromptPreview(retryPrompt)}"));
                        }
                        catch (Exception ex)
                        {
