...
public sealed class HostReleaseDriftWatchdog : BackgroundService
{
        ...
        CancellationToken ct = default)
    {
        ...
        lock (_gate)
        {
            ...
            foreach (var snapshot in _registry.ListCapabilitySnapshots())
            {
                ...
                var entry = new HostReleaseDriftEntry(
                    ...
                    stale);
                entries.Add(entry);
                if (entry.AlarmDue && _alarmed.Add(key)) alarms.Add(entry);
            }
        ...
        {
            _logger.LogWarning(
                "host-release-drift runner={RunnerId} host={HostId} release={ReleaseId} stable={StableVersion} behindHours={BehindHours}",
                alarm.RunnerId,
                alarm.HostId,
                alarm.Release?.ReleaseId,
                stable.Version,
                alarm.BehindForHours);
            await _bus.EmitHostReleaseDriftAsync(
                alarm.HostId,
                alarm.RunnerId,
                alarm.Role,
                alarm.Release?.ReleaseId ?? "unknown",
                stable.Version,
                alarm.BehindForHours ?? 0,
                alarm.Reason,
                ct);
        }
