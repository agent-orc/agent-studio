    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            await foreach (var request in _queue.Reader.ReadAllAsync(stoppingToken))
            {
                ...
                {
                    SilentCatch.Note(__ex, "RemoteReviewEvidenceProjectionWorker: projection cancelled on graceful shutdown.");
                }
        ...
        {
            SilentCatch.Note(__ex, "RemoteReviewEvidenceProjectionWorker: graceful shutdown.");
        }
    }
