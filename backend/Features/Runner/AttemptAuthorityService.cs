...
public sealed class AttemptAuthorityService
{
    ...
    public AttemptWriteResult RenewReview(AttemptWriteReference write, string executorId, int? requestedTtlSeconds)
    {
        lock (_gate)
        {
            ...
            var now = _utcNow();
            var requestedExpiry = now.Add(NormalizeTtl(requestedTtlSeconds));
            if (requestedExpiry > review.Lease.ExpiresAt)
                review.Lease.ExpiresAt = requestedExpiry;
        ...
        string idempotencyKey)
    {
        ...
        lock (_gate)
        {
            ...
            var now = _utcNow();
            // Nothing may take over an unexpired lease. The current instance
            // gets an idempotent answer; a different generation must keep
            // renewing or wait for expiry instead of invalidating live work.
            if (review.State == AttemptLifecycleState.Leased
                && previous.ExpiresAt > now)
            {
                return Same(previous.ClientId, instanceId)
                    ? new AttemptWriteResult(
                        AttemptWriteStatus.Duplicate,
                        review.AttemptId,
                        ReviewAttempt: ToDto(review))
                    : new AttemptWriteResult(
                        AttemptWriteStatus.InvalidState,
                        review.AttemptId,
                        "A different executor instance still holds an unexpired review lease.",
                        ReviewAttempt: ToDto(review));
            }
        ...
        lock (_gate)
        {
            ...
            while (guard-- > 0)
            {
                ...
                if (candidate.Lease is { } expiredLease && expiredLease.ExpiresAt <= now)
                {
                    ...
                    // LeaseExpired to the poller once that same executor
                    // instance polled again (its own delivery key already
                    // recorded the dead lease as its own claim, so the replay
                    // classifier kept answering LeaseExpired and the daemon
                    // crash-looped into full re-registration). Settlement is
                    // fenced - an eventual late report from the old executor
                    // resolves Superseded once this attempt is no longer
                    // current - so double execution costs wasted work, not
                    // correctness, and the poller can move straight to the
                    // next claimable attempt instead.
