...
public sealed class AttemptAuthorityService
{
        ...
        AgentStudio.TaskServer.Contracts.ReviewPlanDto? effectivePlan = null)
    {
        ...
        lock (_gate)
        {
            ...
            // The immutable ReviewSubject keeps route intent. The exact command
            // plan handed to this fenced execution belongs to the attempt so a
            // restart/re-adoption never recomputes it after quota changes.
        ...
        Func<ReviewAttemptDto, AgentStudio.TaskServer.Contracts.ReviewPlanDto?>? resolveEffectivePlan = null)
    {
        ...
        lock (_gate)
        {
            ...
            {
                var effectivePlan = resolveEffectivePlan?.Invoke(ToDto(candidate));
                if (resolveEffectivePlan is not null && effectivePlan is null)
                    continue;
