...
public static class V1ReviewPlaneEndpoints
{
    ...
    public static void MapV1ReviewPlaneEndpoints(this WebApplication app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            if (subjectTask is not null)
                subject = ApplyQuotaFallbackToAspects(
                    ...
                    out quotaDeferredReason);
            else
                quotaDeferredReason = null;
            if (!string.IsNullOrWhiteSpace(quotaDeferredReason))
            {
            ...
            CancellationToken ct) =>
        {
            ...
            var reviewFollowUp = Contract.ReviewFollowUpPolicy.Decide(
                ...
                concernRoundsUsed: 0,
                maxConcernRounds: 1);
            var aspectRetryExhausted =
                reviewFollowUp.Action == Contract.ReviewFollowUpAction.RetryAspect;
            if (aspectRetryExhausted)
            {
                // RemoteReviewWorkspace already reran each malformed aspect once.
                // A surviving RetryAspect result is review infrastructure, not a
                // concern about the delivered code and not another coding round.
                request = ClassifyRetryAspectExhaustion(request, reviewFollowUp);
            }
            ...
            // report timeout. Enqueueing is deferred until after the lane
            // transition below (on every path, including its error returns)
            // so the worker's own fresh task-folder lookup never races this
            // same request's synchronous lane move; enqueueing at settlement
            // instead would win that race often enough to force retries, and
            // every path still enqueues exactly once because a replay of this
            // idempotency key answers Duplicate before reaching here again.
            var evidenceFile = RemoteReviewReportEvidence.EvidenceFileName(attemptId);
            void EnqueueEvidenceProjection() => evidenceQueue.Enqueue(new RemoteReviewEvidenceProjectionRequest(
                attemptId,
            ...
            else
            {
                ...
                if (string.Equals(task.State, TaskStates.AutoReview, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    var category = intervention is not null
                    ...
                    var escalationReason = intervention?.WaitReason
                        ?? (aspectRetryExhausted
                            ? $"Remote Review retried the malformed aspect once, but it still produced no parseable verdict. {chain.Headline}"
    ...
    /// (<see cref="QuotaAdmissionService"/>), the same source the local
        ...
        out string? deferredReason)
    {
        ...
        QuotaAdmissionPlan? lastSwitch = null;
        ...
        var resolvedCommands = subject.Plan.Commands.Select(command =>
        {
            ...
            var plan = quotaAdmission.Plan(new QuotaAdmissionRequest(
                command.CliType,
                command.Model,
                command.ThinkingLevel,
                task.ProjectName,
                OccupiedSlots: 0,
                ExecutionPath: "remote-review-claim"));
            recorder.EmitAdmissionDecision(task, plan, source: "review-claim");
            if (!plan.ShouldLaunch)
            {
                deferred ??= plan.Reason;
                return command;
            }
            if (!plan.IsFallback) return command;
            switched = true;
            lastSwitch = plan;
    ...
    internal static Contract.ReviewReportRequest ClassifyRetryAspectExhaustion(
        Contract.ReviewReportRequest request,
        Contract.ReviewFollowUpDecision decision)
        => request with
        {
            Outcome = "ReviewInfra",
