        ...
        lock (_gate)
        {
            ...
            if (existing is null)
            {
                ...
                var taskId = _mutations.CreateJob(new CreateTaskRequest
                {
                    ...
                    Agent = TimelineActors.Orchestrator,
...
PRODUCT means a defect in the delivery. INFRASTRUCTURE means provider, toolchain,
repository, network, host, or configuration. Return only one word.

Code: {evidence.FailureCode}
Outcome: {evidence.Outcome}
Exit code: {evidence.ExitCode}
Duration ms: {evidence.DurationMs}
Stdout/stderr: {FailureInterventionPolicy.NormalizeSignature((evidence.StdoutTail ?? "") + "\n" + (evidence.StderrTail ?? ""))}
""";
