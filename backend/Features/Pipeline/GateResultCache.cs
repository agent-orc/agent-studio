...
public sealed class GateResultCache
{
        ...
        string toolchainIdentity)
    {
        var payload = JsonSerializer.Serialize(new
        {
            ...
            ChangedFileStatuses = request.ChangedFileStatuses?
    ...
    public static string LocalToolchainIdentity(
        IReadOnlyList<VerifyCommand> commands, string repositoryPath)
        => LocalToolchainIdentity(commands, repositoryPath, ReadToolVersion);

    internal static string LocalToolchainIdentity(
        IReadOnlyList<VerifyCommand> commands, string repositoryPath,
        Func<string, string, string, string> readVersion)
    {
        ...
        foreach (var name in names)
        {
            ...
            // The dotnet host is stable across side-by-side SDK changes. Ask it
            // from each command directory so global.json selects the same SDK
            // that the gate will use. Other versioned tools can change behind
            // an unchanged launcher or symlink as well.
            if (name is "dotnet" or "node" or "npm")
            {
                var directories = name == "dotnet"
                    ? commands.Select(command => BuildTestGateRunner.ResolveWorkingDirectory(repositoryPath, command))
                        .Append(repositoryPath).Distinct(StringComparer.Ordinal)
                    : [repositoryPath];
                foreach (var directory in directories)
                    parts.Add($"{name}:{Path.GetRelativePath(repositoryPath, directory)}:{readVersion(path, directory, "--version")}");
            }
        }
    ...
    private static string ReadToolVersion(string executable, string directory, string argument)
    {
        ...
        {
            StartInfo = new ProcessStartInfo(executable, argument)
            {
        ...
        if (!process.Start()) throw new IOException($"Could not start toolchain probe: {executable}");
        var stdout = process.StandardOutput.ReadToEndAsync();
        var stderr = process.StandardError.ReadToEndAsync();
        if (!process.WaitForExit(5000))
        {
            process.Kill(entireProcessTree: true);
            throw new IOException($"Toolchain probe timed out: {executable}");
        }
        var output = stdout.GetAwaiter().GetResult().Trim();
        var error = stderr.GetAwaiter().GetResult().Trim();
        if (process.ExitCode != 0 || output.Length + error.Length > 4096)
            throw new IOException($"Toolchain probe failed: {executable}");
        return output + "\n" + error;
    }
