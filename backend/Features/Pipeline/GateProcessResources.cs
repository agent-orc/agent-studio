...
internal enum GateResourceOperatingSystem
{
    Other,
    Linux,
    Windows,
...
internal readonly record struct GateWindowsProcessEntry(int ProcessId, int ParentProcessId);

/// <summary>
/// The OS-specific snapshot boundary. Its internal constructor lets tests feed
/// recorded Windows Toolhelp and GetSystemTimes values through the same branch
/// while running on Linux.
/// </summary>
internal sealed class GateProcessResourcePlatform
{
    private readonly GateResourceOperatingSystem _operatingSystem;
    private readonly Func<SystemLoadThrottle.CpuTicks?> _readHostCpu;
    private readonly Func<IReadOnlyList<GateWindowsProcessEntry>> _readWindowsProcesses;

    private GateProcessResourcePlatform()
    {
        _operatingSystem = OperatingSystem.IsLinux()
            ? GateResourceOperatingSystem.Linux
            : OperatingSystem.IsWindows()
                ? GateResourceOperatingSystem.Windows
                : GateResourceOperatingSystem.Other;
        _readHostCpu = ReadHostCpu;
        _readWindowsProcesses = ReadNativeWindowsProcesses;
        ProcessorCount = _operatingSystem == GateResourceOperatingSystem.Linux
            ? File.ReadLines("/proc/stat").Count(line =>
                line.Length > 3 && line.StartsWith("cpu", StringComparison.Ordinal) && char.IsDigit(line[3]))
            : Environment.ProcessorCount;
    }

    internal GateProcessResourcePlatform(
        GateResourceOperatingSystem operatingSystem,
        Func<SystemLoadThrottle.CpuTicks?> readHostCpu,
        Func<IReadOnlyList<GateWindowsProcessEntry>> readWindowsProcesses,
        int processorCount)
    {
        _operatingSystem = operatingSystem;
        _readHostCpu = readHostCpu;
        _readWindowsProcesses = readWindowsProcesses;
        ProcessorCount = Math.Max(1, processorCount);
    }

    internal static GateProcessResourcePlatform Current { get; } = new();
    internal int ProcessorCount { get; }
    internal SystemLoadThrottle.CpuTicks? ReadHostCpuTicks() => _readHostCpu();

    internal Dictionary<int, int> ReadParents()
    {
        var result = new Dictionary<int, int>();
        if (_operatingSystem == GateResourceOperatingSystem.Linux)
        {
        ...
        else if (_operatingSystem == GateResourceOperatingSystem.Windows)
        {
            foreach (var entry in _readWindowsProcesses())
                result[entry.ProcessId] = entry.ParentProcessId;
        }
        return result;
    }

    private static SystemLoadThrottle.CpuTicks? ReadHostCpu()
        => SystemLoadThrottle.SystemCpuReader.TryRead(out var ticks) ? ticks : null;

    private static IReadOnlyList<GateWindowsProcessEntry> ReadNativeWindowsProcesses()
    {
        var result = new List<GateWindowsProcessEntry>();
        ...
        try
        {
            ...
            do { result.Add(new((int)entry.ProcessId, (int)entry.ParentProcessId)); }
...
internal sealed class GateProcessResources : IGateProcessResources
{
    ...
    private readonly GateProcessResourcePlatform _platform;
    ...
    internal GateProcessResources(int rootPid)
        : this(rootPid, GateProcessResourcePlatform.Current)
    {
    }

    internal GateProcessResources(
        int rootPid,
        GateProcessResourcePlatform platform,
        Func<long>? now = null)
    {
        _rootPid = rootPid;
        _platform = platform;
        _now = now ?? (() => _clock.ElapsedMilliseconds);
        _host = _platform.ReadHostCpuTicks();
        Sample();
    }
    ...
    private GateResourceEvidence SampleCore()
    {
        ...
        try
        {
            var parents = _platform.ReadParents();
        ...
        _available = false;
        if (_platform.ReadHostCpuTicks() is { } host)
        {
            if (_host is { } previous && elapsed > 0 && host.Total > previous.Total && host.Idle >= previous.Idle)
            {
                ...
                // Host tick counters span all host CPUs. The platform boundary
                // therefore reports physical host counters and their matching
                // processor count, including recorded Windows samples in tests.
                var cores = _platform.ProcessorCount;
