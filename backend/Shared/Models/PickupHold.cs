...
public static class PickupHoldPolicy
{
    ...
    private static PickupHoldStatus? DependencyHold(PickupHoldFacts facts, DateTime laneEntry)
    {
        ...
        var openDependencies = waitsOn.Items
            .Where(item => !item.Fulfilled)
            .Select(item => new ClassifiedDependency(item, ClassifyDependency(item, facts.NowUtc)))
            .OrderByDescending(item => ClassificationRank(item.Classification))
            .ThenBy(item => item.Item.Key, StringComparer.OrdinalIgnoreCase)
            .ToList();
        if (openDependencies.Count == 0) return null;

        var deciding = openDependencies[0];
        var open = deciding.Item;
        var reasonSuffix = OtherDependencyDetail(openDependencies.Skip(1));

        if (deciding.Classification == PickupHoldClassifications.Unsatisfiable)
        {
            var reason = open.Unsatisfiable
                ? open.UnsatisfiableReason.Length > 0
                    ? open.UnsatisfiableReason
                    : WaitsOnEvaluator.ArchivedGateReason(open.Key)
                : $"{open.Key} does not exist in the workspace, so nothing can fulfil this edge.";
            return Hold(
                PickupHoldMechanisms.DependencyGate,
                reason + reasonSuffix,
                laneEntry, facts.NowUtc, unsatisfiable: true,
                DependencyDecisions(open.Key).ToArray()) with
            {
        ...
        if (open.WaitingForRelease)
            return Hold(
                PickupHoldMechanisms.DependencyGate,
                $"{open.Key} is terminal but carries no explicit release, and a release gate needs one.{reasonSuffix}",
                laneEntry, facts.NowUtc, unsatisfiable: false,
                new PickupHoldResolution(
        ...
        var stalled = deciding.Classification == PickupHoldClassifications.Stalled;
        var hold = HoldClassified(
                PickupHoldMechanisms.DependencyGate,
                $"{open.Key} has not reached a terminal lane yet (currently {Describe(open.TargetState)}).{reasonSuffix}",
                laneEntry, facts.NowUtc, unsatisfiable: false,
        ...
        return hold with
        {
            ...
            AttentionReason = stalled
                ? StalledAttentionReason(open)
                : null,
            AttentionSinceUtc = stalled
                ? open.TargetBlockerSinceUtc ?? open.TargetEnteredLaneAt
                : null,
        };
    }

    private static string ClassifyDependency(WaitsOnItem target, DateTime now)
    {
        if (target.Unsatisfiable || !target.Resolved)
            return PickupHoldClassifications.Unsatisfiable;
        return IsStalled(target, now)
            ? PickupHoldClassifications.Stalled
            : PickupHoldClassifications.SatisfiableSoon;
    }

    private static int ClassificationRank(string classification) => classification switch
    {
        PickupHoldClassifications.Unsatisfiable => 3,
        PickupHoldClassifications.Stalled => 2,
        _ => 1,
    };

    private static string OtherDependencyDetail(IEnumerable<ClassifiedDependency> dependencies)
    {
        var others = dependencies
            .Select(dependency => $"{dependency.Item.Key} ({dependency.Classification})")
            .ToList();
        return others.Count == 0
            ? ""
            : $" Other open dependencies: {string.Join(", ", others)}.";
    }

    private sealed record ClassifiedDependency(WaitsOnItem Item, string Classification);
