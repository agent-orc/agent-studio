...
/// <summary>
/// Lifecycle of a decision card. Orthogonal to the lane state: a
/// pending decision waits in <c>1-preparation</c>, a decided one becomes a
/// durable record, and reopening returns it to pending. Persisted as
...
public static class DecisionStatuses
{
    ...
    /// <summary>Legacy request status accepted when reading an early card.</summary>
    public const string Requested = "requested";
    /// <summary>A choice was recorded; the card is a durable decision record.</summary>
    public const string Decided = "decided";
    /// <summary>History entry for a reopen; the card status returns to pending.</summary>
    public const string Reopened = "reopened";

    public static readonly string[] All = [Pending, Decided];

    /// <summary>Coerce a free-form value to a known status; unknown / empty becomes pending.</summary>
    ...
    /// <summary>True when the decision is pending.</summary>
    public static bool IsOpen(string? value) => Normalize(value) == Pending;
}
...
public sealed record DecisionContent
{
    ...
    /// <summary>Two to four options, each with structured consequences, effort, and risk.</summary>
    ...
    /// <summary>
    /// Project wiki relative path of the ADR-style record written on decide.
    /// </summary>
    public string? RecordPath { get; init; }

    /// <summary>Append-only decision and reopen entries retained across cycles.</summary>
    public List<DecisionHistoryEntry> History { get; init; } = [];
}
...
public enum DecisionCardErrorCode
{
    ...
    /// <summary>A recommendation needs both an option and a reason.</summary>
...
public static class DecisionCardPolicy
{
    ...
    public static bool MayDecide(DecisionContent content, string? clientId, string? role)
    {
        var decider = string.IsNullOrWhiteSpace(content.Decider)
            ? DecisionDeciders.Operator : content.Decider.Trim();
        if (IdComparer.Equals(decider, DecisionDeciders.Operator)) return true;
        if (decider.StartsWith("role:", StringComparison.OrdinalIgnoreCase))
            return IdComparer.Equals(decider[5..], role);
        if (IdComparer.Equals(decider, "owner")) return IdComparer.Equals(role, "owner");
        return IdComparer.Equals(decider, clientId?.Trim());
    }
    ...
    public static IReadOnlyList<DecisionCardError> ValidateContent(DecisionContent? content)
    {
        ...
        if (string.IsNullOrWhiteSpace(content.RecommendedOptionId)
            != string.IsNullOrWhiteSpace(content.RecommendationReason))
            errors.Add(new(DecisionCardErrorCode.IncompleteRecommendation,
                "A recommendation needs both an option id and a reason."));
    ...
    /// open and the option must exist. The decider may leave the rationale empty.
    /// </summary>
    ...
    public static IReadOnlyList<DecisionCardError> ValidateReopen(DecisionContent content, string? note)
    {
        var errors = new List<DecisionCardError>();
        ...
        if (string.IsNullOrWhiteSpace(note))
            errors.Add(new(DecisionCardErrorCode.MissingReopenNote, "Explain why the decision is being reopened."));
        return errors;
    }
