...
public class TaskBatchMoveTests : IDisposable
{
    ...
    [InlineData(true)]
    [Trait("Category", "MachineBound")]
    public async Task ArchiveOverride_EvidenceFailure_RestoresCompletedLane(bool blockAudit)
    {
        using var factory = new WebApplicationFactory<Program>().WithWebHostBuilder(builder =>
        {
            builder.UseEnvironment("Test");
        ...
        // Let the startup reconciler finish before adding this deliberately
        // unintegrated coding card. The override must work for that case.
        var scanner = factory.Services.GetRequiredService<TaskScannerService>();
        WriteJob(TaskStates.Completed, "override-evidence-failed");
        var original = Path.Combine(_watchPath, TaskStates.Completed, "override-evidence-failed");
        scanner.InvalidateCache();
        if (blockAudit)
            File.WriteAllText(Path.Combine(_watchPath, ".audit"), "blocked");
        else
        {
            Directory.CreateDirectory(Path.Combine(original, "logs"));
            Directory.CreateDirectory(Path.Combine(original, "logs", "timeline.jsonl"));
        }
        var cardBeforeMove = scanner.FindJob("override-evidence-failed", _watchPath)!;
        var authority = factory.Services.GetRequiredService<AttemptAuthorityService>();
        var run = authority.AcquireRun(cardBeforeMove.TaskKey, "PROJ-002", null,
            "runner-a", "host-a", 60, "archive-override-run").RunAttempt!;
        Assert.NotNull(run);
        Assert.True(authority.SettleRun(new SettleRunAttemptRequest
        {
            Write = new AttemptWriteReference(run.AttemptId, run.LastFence,
                run.AuthorityEpoch, "archive-override-settle"),
            Outcome = "done",
            ResultSha = "archive-override-result",
        }).Accepted);
        var review = authority.CreateReviewAttempt(new CreateReviewAttemptRequest(
            cardBeforeMove.TaskKey, "PROJ-002", "archive-override-result",
            run.AttemptId, "requirements", "policy", [], "archive-override-review"))
            .ReviewAttempt!;
        Assert.NotNull(review);
        var transition = factory.Services.GetRequiredService<TaskTransitionService>();
        var outcome = await transition.MoveAsync("override-evidence-failed", TaskStates.Archive,
            _watchPath, cause: TimelineActors.Human("owner"),
            reason: "Operator accepted archival with an unrecovered delivery.",
            archiveOverride: true);
        Assert.Equal(MoveJobStatus.Failure, outcome.Status);
        var card = factory.Services.GetRequiredService<TaskScannerService>()
            .FindJob("override-evidence-failed", _watchPath);
        Assert.Equal(TaskStates.Completed, card?.State);
        Assert.Equal(AttemptLifecycleState.Pending, authority.GetReview(review.AttemptId)?.State);
        Assert.False(Directory.Exists(Path.Combine(_watchPath, TaskStates.Archive,
            "override-evidence-failed")));
        Assert.False(File.Exists(Path.Combine(_watchPath, ".audit", "archive-overrides.jsonl")));
        if (blockAudit)
            Assert.DoesNotContain(factory.Services.GetRequiredService<TimelineLog>()
                .ReadAll(card!.FolderPath), row => row.Kind == "archive_override");
