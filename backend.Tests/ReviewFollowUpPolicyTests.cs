...
public class ReviewFollowUpPolicyTests
{
    ...
    [Fact]
    public void Mixed_block_and_unparseable_verdict_prioritizes_review_infrastructure()
    {
        var decision = ReviewFollowUpPolicy.Decide(
            [
                new("requirement-fit", "block", "A real product finding.", Finding: "Fix the contract."),
                new("code-quality", "concerns", "No parseable verdict.", Classification: "review:unparseable"),
            ],
            concernRoundsUsed: 0,
            maxConcernRounds: 1);

        Assert.Equal(ReviewGrade.ProductFailure, decision.Grade);
        Assert.Equal(ReviewFollowUpAction.RetryAspect, decision.Action);
        Assert.Equal("code-quality", Assert.Single(decision.Findings).Aspect);
        Assert.Contains("code-quality", decision.Reason);
    }
    ...
    [Fact]
    public void Local_consumer_preserves_a_block_while_marking_only_the_unparseable_aspect_as_infrastructure()
    {
        var block = new AspectVerdict("requirement-fit", AspectStatus.Block, "Contract mismatch.", "body", "requirement:block");
        var unparseable = new AspectVerdict("code-quality", AspectStatus.Concerns, "No parseable verdict.", "body", "review:unparseable");
        var decision = ReviewFollowUpPolicy.Decide(
            [
                new(block.Aspect, "block", block.Summary, Classification: block.ConcernTagId),
                new(unparseable.Aspect, "concerns", unparseable.Summary, Classification: unparseable.ConcernTagId),
            ], 0, 1);

        var report = ReviewDecisionOrchestrator.MarkRetryAspectAsInfrastructure(
            AspectRunReport.From([block, unparseable]), decision);

        Assert.True(report.HasInfraFailure);
        Assert.False(report.Verdicts.Single(item => item.Aspect == "requirement-fit").IsInfraFailure);
        Assert.True(report.Verdicts.Single(item => item.Aspect == "code-quality").IsInfraFailure);
    }
    ...
    [Fact]
    public void Remote_consumer_classifies_mixed_block_and_unparseable_as_review_infrastructure()
    {
        var request = new ReviewReportRequest(
            "reviewer", "instance", "lease", 1, "report", "ProductFailure", null,
            "A product finding also exists.", null!, null!, [], [], []);
        var decision = ReviewFollowUpPolicy.Decide(
            [
                new("requirement-fit", "block", "Contract mismatch."),
                new("code-quality", "concerns", "No parseable verdict.", Classification: "review:unparseable"),
            ], 0, 1);
        ...
        Assert.Equal("ReviewInfra", classified.Outcome);
        Assert.Equal("AspectVerdictUnparseable", classified.FailureClassification);
        Assert.Contains("code-quality", classified.Summary);
    }
