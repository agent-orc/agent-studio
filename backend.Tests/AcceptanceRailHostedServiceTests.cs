        ...
        string state = TaskStates.HumanReview)
    {
        ...
        if (conflict || infrastructureFailure)
        {
            ...
            stack.Pipeline.RecordStep(folder, new PipelineStepExecution
            {
                ...
                StartedAt = now,
                CompletedAt = now,
                Verdict = infrastructureFailure ? "gate-failed" : "conflict",
                ...
                // Verbatim 2026-09-06 evidence: a 30-second git network cap, which
                // says nothing about the reviewed change.
