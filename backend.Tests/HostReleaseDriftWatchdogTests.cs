...
public sealed class HostReleaseDriftWatchdogTests : IDisposable
{
    ...
    [Fact]
    public async Task An_identical_short_commit_does_not_suppress_the_drift_alarm()
    {
        var shortCommitRelease = OldRelease with { Commit = "abcdef" };
        Advertise("runner-short-commit", release: shortCommitRelease);
        var watchdog = NewWatchdog(StableBuild with { Commit = "abcdef" });

        var first = await watchdog.RefreshAsync();
        var second = await watchdog.RefreshAsync();

        var host = Assert.Single(first.Hosts);
        Assert.Equal(HostReleaseDriftStates.Behind, host.State);
        Assert.True(host.AlarmDue);
        Assert.Equal(HostReleaseDriftStates.Behind, Assert.Single(second.Hosts).State);
        Assert.Single(_store.Recent(_workspace, null, 20), message => message.Topic == "host_release_drift");
    }
