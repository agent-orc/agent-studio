...
public sealed class SummaryInputBuilderTests
{
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void Build_UsesReviewSubjectForNetDiffStat_WhenSecondCommitRevertsPartOfFirst(
        bool hasAttributedCommit)
    {
        var root = TempFolder();
        ...
        try
        {
            ...
            {
                IntegrationBranch = "refs/heads/main",
                Commits = hasAttributedCommit ? [new TaskCommitInfo
                {
                    Sha = tip,
                    ShortSha = tip[..7],
            ...
            if (!hasAttributedCommit)
                Assert.Contains("Attributed commits: none recorded.", inputs.Delivery, StringComparison.Ordinal);
        }
        finally { Directory.Delete(root, true); }
    }
