...
public sealed class TaskCoreCacheTests : IDisposable
{
    ...
    [Fact]
    public void DependencyMutations_RepublishDependentsBeforeAcknowledgment_WithoutScanning()
    {
        var target = MakeTask("AGT-target", TaskStates.Ready);
        var dependent = MakeTask("AGT-dependent", TaskStates.Ready) with
        {
            References = new TaskReferences
            {
                DependsOn = [new TaskDependencyReference(target.Key!, releaseGate: true)],
            },
        };
        var unrelated = MakeTask("AGT-unrelated", TaskStates.Ready);
        var scans = 0;
        var cache = Cache(() => { Interlocked.Increment(ref scans); return [target, dependent, unrelated]; });
        cache.GetSnapshot();
        var before = cache.GetCore(dependent.Id, _root).Record!;
        var unrelatedVersion = cache.GetCore(unrelated.Id, _root).Record!.Version;
        Assert.True(before.DependencyBlocked);
        Assert.False(Assert.Single(before.Dependencies).WaitingForRelease);

        cache.Invalidate();
        cache.PublishCore(target with { State = TaskStates.Archive });
        var afterArchive = cache.GetCore(dependent.Id, _root).Record!;
        Assert.Equal("ready", afterArchive.DependencyState);
        Assert.True(Assert.Single(afterArchive.Dependencies).Unsatisfiable);
        cache.PublishCore(target with { State = TaskStates.Archive, Released = true });
        var afterRelease = cache.GetCore(dependent.Id, _root).Record!;
        Assert.Equal("ready", afterRelease.DependencyState);
        Assert.False(afterRelease.DependencyBlocked);
        Assert.True(Assert.Single(afterRelease.Dependencies).Fulfilled);
        Assert.NotEqual(before.Version, afterRelease.Version);
        Assert.Equal(unrelatedVersion, cache.GetCore(unrelated.Id, _root).Record!.Version);
        Assert.Equal(1, scans);

        cache.PublishCore(dependent with { References = new TaskReferences() });
        Assert.Empty(cache.GetCore(dependent.Id, _root).Record!.Dependencies);
        cache.PublishCore(dependent);
        cache.RemoveCore(target);
        var afterDelete = cache.GetCore(dependent.Id, _root).Record!;
        Assert.Equal("ready", afterDelete.DependencyState);
        Assert.True(afterDelete.DependencyBlocked);
        Assert.False(Assert.Single(afterDelete.Dependencies).Resolved);
        Assert.Equal(1, scans);
    }
