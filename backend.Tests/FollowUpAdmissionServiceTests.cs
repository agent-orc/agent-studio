...
public sealed class FollowUpAdmissionServiceTests : IDisposable
{
    ...
    [Fact]
    public void StartupReconciliation_ConvertsOnlyIntentOlderThanNewestRun()
    {
        WriteJob("stale", TaskStates.HumanReview);
        WriteJob("queued", TaskStates.HumanReview);
        var harness = Build();
        harness.Mutations.SavePendingIntent(
            "stale", ContinueModes.Steer, "Already delivered.", "remote-execution",
            activeJobId: null, watchPath: _watchPath);
        harness.Sessions.AppendSessionEvent(
            "stale",
            new SessionEvent
            {
                Ts = DateTime.UtcNow.AddSeconds(1),
                Kind = "continue",
                Cli = "remote-runner",
                RunAttemptId = "run-stale",
            },
            _watchPath);
        harness.Sessions.AppendSessionEvent(
            "queued",
            new SessionEvent
            {
                ...
                Kind = "continue",
                Cli = "remote-runner",
                RunAttemptId = "run-before-intent",
            },
            _watchPath);
        harness.Mutations.SavePendingIntent(
            "queued", ContinueModes.Continue, "Still queued.", "remote-execution",
            activeJobId: null, watchPath: _watchPath);

        var outcome = harness.Transitions.ReconcilePendingIntents();

        Assert.Equal(2, outcome.Inspected);
        Assert.Equal(1, outcome.Delivered);
        Assert.Empty(outcome.Failures);
        var staleFolder = Path.Combine(_watchPath, TaskStates.HumanReview, "stale");
        Assert.Null(ReadIntent(staleFolder));
        Assert.Contains(
            harness.Timeline.ReadAll(staleFolder),
            row => row.Kind == TimelineEventKinds.FollowUpConsumed
                   && row.RunId == "run-stale");
        Assert.Equal(
            "Still queued.",
            ReadIntent(Path.Combine(_watchPath, TaskStates.HumanReview, "queued"))!.Prompt);
    }
