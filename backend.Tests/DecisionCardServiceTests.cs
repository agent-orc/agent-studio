...
public class DecisionCardServiceTests : IDisposable
{
    ...
    public async Task GenericMove_CannotReopenDecidedCard_OrSkipItsAuditTrail()
    {
        ...
        var jobId = h.Mutations.CreateJob(new CreateTaskRequest
        {
            Id = "audited-reopen", Title = "Audited reopen", WatchPath = _watchPath,
            Kind = TaskKinds.Decision, Decision = SampleContent(),
        })!;
        ...
        var timelineCount = timelineLog.ReadAll(decided.FolderPath).Count;
        var activityCount = h.ActivityFeed.Read(_watchPath).Count;
        ...
        Assert.Equal(DecisionCardStatus.Success, (await h.Decisions.ReopenAsync(jobId, _watchPath,
    ...
    private Harness Build()
    {
        ...
        return new Harness(scanner, mutations, decisions, states, transitions, activityFeed);
    }
