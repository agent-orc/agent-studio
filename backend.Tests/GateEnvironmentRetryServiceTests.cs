...
public sealed class GateEnvironmentRetryServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task RetryNow_WhileAnAcceptanceIntegrationIsInFlight_IsRefusedByTheSamePolicyAsTheSweep()
    {
        var stack = Build();
        SeedGateEnvironmentFailure(stack, "in-flight", failedMinutesAgo: 600);
        // Exactly the state the accepted-integration backstop drives a card
        // through: the acceptance transaction already owns this merge.
        SetPhase(stack, "in-flight", LifecyclePhases.Integrating);
        var job = stack.Scanner.FindJob("in-flight", _watchPath)!;

        var result = await stack.Retries.RetryNowAsync(job);

        // The button overrides the backoff, never the eligibility: a second
        // merge into the integration branch would race the acceptance one.
        Assert.Equal(GateEnvironmentRetryStatus.NotApplicable, result.Status);
        Assert.Equal(GateEnvironmentRetryReasons.AcceptanceIntegrationInFlight, result.Code);
        Assert.Equal(0, stack.Gate.Invocations);
        Assert.Empty(Receipts(stack, "in-flight"));
    }

    [Fact]
    public async Task RetryNow_WhenTheLatestReviewForTheDeliveryShaFailed_IsRefusedDespiteTheOlderPass()
    {
        var stack = Build();
        var delivery = SeedGateEnvironmentFailure(stack, "overturned", failedMinutesAgo: 600);
        // The same delivery was reviewed again and the second round found a
        // product failure. The stale Pass must not license an integration.
        SettleAnotherReview(stack, "overturned", delivery, ReviewTerminalOutcome.ProductFailure);
        var job = stack.Scanner.FindJob("overturned", _watchPath)!;
    ...
    [Fact]
    public async Task Sweep_WhenTheLatestReviewForTheDeliveryShaFailed_NeverReplays()
    {
        var stack = Build();
        var delivery = SeedGateEnvironmentFailure(stack, "overturned-sweep", failedMinutesAgo: 600);
        SettleAnotherReview(stack, "overturned-sweep", delivery, ReviewTerminalOutcome.ProductFailure);
        ...
        Assert.Equal(0, sweep.Retried);
        Assert.Equal(0, stack.Gate.Invocations);
        Assert.False(IsAncestor(delivery, "develop"));
    }

    [Fact]
    public async Task RetryNow_WhenTheLatestReviewForTheDeliveryShaPassedAgain_Replays()
    {
        var stack = Build();
        var delivery = SeedGateEnvironmentFailure(stack, "re-passed", failedMinutesAgo: 600);
        // The mirror of the overturned case: a later round settled Pass again,
        // so "latest settled" is still a green light.
        SettleAnotherReview(stack, "re-passed", delivery, ReviewTerminalOutcome.Pass);
        var job = stack.Scanner.FindJob("re-passed", _watchPath)!;
    ...
    /// <summary>
    /// Settles one more review round for the same delivery SHA, which is what a
    /// re-review of an unchanged delivery produces in attempt authority.
    /// </summary>
    private static void SettleAnotherReview(
        Stack stack,
        string id,
        string deliverySha,
        ReviewTerminalOutcome outcome)
    {
        ...
        var suffix = $"{id}-{outcome}";
        var review = stack.Authority.CreateReviewAttempt(new CreateReviewAttemptRequest(
            taskKey, "PROJ-001", deliverySha, run.AttemptId,
            "requirements", "policy", [], "review-create-" + suffix)).ReviewAttempt!;
        var claimed = stack.Authority.ClaimReview(
            review.AttemptId, "reviewer", "review-host", 600, "review-claim-" + suffix).ReviewAttempt!;
        var settled = stack.Authority.SettleReview(new SettleReviewAttemptRequest(
            new AttemptWriteReference(
                claimed.AttemptId, claimed.LastFence, claimed.AuthorityEpoch, "review-settle-" + suffix),
            deliverySha,
            outcome));
        Assert.Equal(AttemptWriteStatus.Accepted, settled.Status);
    }

    /// <summary>Rewrites the card's lifecycle phase in place, as the rails do.</summary>
    private void SetPhase(Stack stack, string id, string phase)
    {
        var path = Path.Combine(stack.Scanner.FindJob(id, _watchPath)!.FolderPath, "task.json");
        using var document = JsonDocument.Parse(File.ReadAllText(path));
        var fields = document.RootElement.EnumerateObject()
            .Where(property => property.Name != "phase")
            .ToDictionary(property => property.Name, property => property.Value.Clone());
        fields["phase"] = JsonDocument.Parse($"\"{phase}\"").RootElement.Clone();
        File.WriteAllText(
            path,
            JsonSerializer.Serialize(fields, new JsonSerializerOptions(JsonSerializerDefaults.Web)
            {
                WriteIndented = true,
            }));
    }
