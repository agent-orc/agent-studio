...
///   <item>a <c>ProcessRunner.RunAsync</c> call in a file that declares
///   <c>CliSelection</c> or <c>DetachedJobSpec</c>, even when its binary name
///   moved into a serialized spec record (T0b).</item>
/// </list>
...
public class CliInvocationCentralizationGuardTests
{
    ...
    /// <summary>
    /// Runner invocation-spec types. A ProcessRunner call in such a file is a
    /// raw CLI launch even when the binary name is stored far from the call.
    /// The detached worker's Process.Start launches the runner host itself;
    /// only ProcessRunner calls use this file-scoped rule.
    /// </summary>
    private static readonly Regex FileScopedCliIdentity = new(
        @"\b(?:CliSelection|DetachedJobSpec)\b",
        RegexOptions.Compiled);
    ...
    /// <summary>
    /// One bounded non-card process utility. <paramref name="Justification"/>
    /// must explain why it remains outside CAR.
    /// </summary>
    ...
    /// <summary>
    /// Bounded non-card process utilities. Adding a card-run launch here is an
    /// architecture regression; remove entries when their utilities disappear.
    /// </summary>
    ...
    [
        ...
        new("backend/Features/Cli/Execution/NpmShimHealer.cs",
            DocumentedException
            + "The non-agent ClaudeOneShot alone retains this repair helper. CAR owns repair "
            + "for card runs; AGT-2373 removed the local rollback path."),
    ...
    [Fact]
    public void Guard_catches_spec_driven_raw_runner_call_without_a_nearby_cli_name()
    {
        const string source = """
            internal record DetachedJobSpec(string FileName, string Arguments);
            internal static class Launcher
            {
                internal static Task<ProcessResult> Run(DetachedJobSpec spec)
                    => ProcessRunner.RunAsync(spec.FileName, spec.Arguments);
            }
            """;

        var violations = ScanSource("runner/NewLauncher.cs", source);
        Assert.Contains(violations, violation => violation.Kind == "cli-spawn"
            && violation.Evidence.Contains("coding-agent invocation spec", StringComparison.Ordinal));
    }
    ...
    private static IReadOnlyList<CliInvocationViolation> ScanSource(string relativePath, string source)
    {
        ...
        for (var i = 0; i < code.Length; i++)
        {
                ...
                && !IsVersionOnlyAvailabilityProbe(relativePath, code, i))
            {
                ...
                evidence ??= declaresInvocationSpec
                    && code[i].Contains("ProcessRunner.RunAsync", StringComparison.Ordinal)
                    ? "the file declares a coding-agent invocation spec" : null;
