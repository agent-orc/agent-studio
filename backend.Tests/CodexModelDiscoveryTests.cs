...
public class CodexModelDiscoveryTests : IDisposable
{
    ...
    [Fact]
    public void ParseDebugModelsJson_TakesTheReasoningLadderAndDefaultFromTheCli()
    {
        ...
        Assert.DoesNotContain(models, m => m.ThinkingLevels.Contains("minimal"));
        // A CLI-stated default wins over the AGT-2025 top-of-ladder rule.
        Assert.Equal("low", Assert.Single(models, m => m.Id == ModelIds.Gpt56Sol).DefaultThinkingLevel);
        Assert.Equal("medium", Assert.Single(models, m => m.Id == ModelIds.Gpt55).DefaultThinkingLevel);
