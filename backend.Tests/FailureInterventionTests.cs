...
public sealed class FailureInterventionTests : IDisposable
{
    ...
    {
        var (scanner, mutations, service, log, timeline) = Build();
        ...
        WriteEvidenceSnapshots(item, followUp, firstRoundTrip, followUpTimeline, originTimeline, feed, reportLine);
    }
        ...
        string reportLine)
    {
        ...
        File.WriteAllText(Path.Combine(results, "driving-incident-replay.json"),
            JsonSerializer.Serialize(new { intervention = item, followUp, origin, followUpTimeline, originTimeline, feed },
    ...
    private (TaskScannerService Scanner, TaskMutationService Mutations, FailureInterventionService Service, OrchestratorLog Log, TimelineLog Timeline) Build()
    {
        ...
        return (scanner, mutations, service, log, timeline);
    }
