...
public sealed class CliQuotaFallbackServiceTests : IDisposable
{
    ...
    [Fact]
    public void Resolve_ProviderCap_RejectsSameFamilyOverride()
    {
        var service = NewService();
        service.Set(new CliModelRouteProfile
        {
            ...
            FallbackModel = ModelIds.ClaudeSonnet5,
        });

        var decision = service.Resolve(CliTypes.Claude, ModelIds.ClaudeOpus5, "high", _ =>
            new CapEvaluation(true, CliTypes.Claude, "Weekly", 95, 100));

        Assert.False(decision.IsFallback);
        Assert.Equal(ModelIds.ClaudeOpus5, decision.Model);
        Assert.Contains("same-family fallback", decision.Reason);
    }
    ...
    [Fact]
    public void Resolve_ClaudeQuotaFull_DerivesCodexFallbackWithoutCycling()
    {
        var service = NewServiceWithCatalog();
    ...
    [Fact]
    public void Resolve_ExplicitThinkingAboveCatalogueTier_DoesNotLowerCapabilityFloor()
    {
        var service = NewServiceWithCatalog();

        var decision = service.Resolve(CliTypes.Codex, ModelIds.Gpt56Sol, "xhigh", cli =>
            cli == CliTypes.Codex
                ? new CapEvaluation(true, CliTypes.Codex, "Weekly", 95, 100)
                : CapEvaluation.NotBlocked);
        ...
        Assert.False(decision.IsFallback);
        Assert.Equal(ModelIds.Gpt56Sol, decision.Model);
        Assert.Equal("xhigh", decision.ThinkingLevel);
    }
