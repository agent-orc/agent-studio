...
public sealed class HostReleaseDriftPolicyTests
{
    ...
    [Fact]
    public void Identical_six_character_commits_do_not_match()
        => Assert.False(HostReleaseDriftPolicy.CommitsMatch("abcdef", "abcdef"));

    [Fact]
    public void A_seven_character_commit_prefix_matches_a_full_sha()
        => Assert.True(HostReleaseDriftPolicy.CommitsMatch("abcdef0", "abcdef0123456789abcdef0123456789abcdef01"));

    [Theory]
    [InlineData("notasha", "notasha")]
    [InlineData("abcdefg", "abcdef0123456789abcdef0123456789abcdef01")]
    public void Non_hex_commit_values_do_not_match(string left, string right)
        => Assert.False(HostReleaseDriftPolicy.CommitsMatch(left, right));

    [Fact]
    public void An_equal_version_with_an_identical_short_commit_is_unknown()
    {
        var verdict = HostReleaseDriftPolicy.Evaluate(
            new Contract.RunnerReleaseIdentityDto("agt-host-local", "0.3.0", "abcdef"),
            new StableReleaseIdentity("0.3.0", "abcdef", null),
            Now);
