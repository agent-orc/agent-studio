...
public sealed class OperationsTests
{
    ...
    [Fact]
    public async Task Complete_temporary_report_is_replayed_before_polling_without_reexecuting()
    {
        await using var f = await Fixture.Start();
        var spool = Path.Combine(f.Directory, "spool");
        var agent = new OperationsAgentClient(f.Agent, "host-a", "boot-a", spool, f.Clock);
        await agent.RegisterAsync(default);
        await f.Submit();
        var dispatched = (await f.Poll())!;
        var report = f.Report(dispatched) with
        {
            Result = f.Report(dispatched).Result with
            {
                Output = JsonSerializer.SerializeToElement(new { os = "recovered", architecture = "x64", processors = 1 })
            }
        };
        await File.WriteAllTextAsync(Path.Combine(spool, dispatched.Id + ".json.tmp"),
            JsonSerializer.Serialize(report, OperationsProtocol.Json));

        await agent.PollOnceAsync(default);

        var completed = await f.Service.GetFromJsonAsync<OperationAttempt>(Fixture.Api + $"attempts/{dispatched.Id}");
        Assert.Equal("succeeded", completed!.State);
        Assert.Equal("recovered", completed.Result!.Output.GetProperty("os").GetString());
        Assert.Empty(System.IO.Directory.EnumerateFiles(spool));
    }

    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public async Task New_boot_replays_previous_boot_report_before_registration(bool interruptedWrite)
    {
        await using var f = await Fixture.Start();
        var spool = Path.Combine(f.Directory, "spool");
        await f.Register();
        await f.Submit();
        var dispatched = (await f.Poll())!;
        var report = f.Report(dispatched);
        System.IO.Directory.CreateDirectory(spool);
        var path = Path.Combine(spool, dispatched.Id + ".json" + (interruptedWrite ? ".tmp" : ""));
        await File.WriteAllTextAsync(path, JsonSerializer.Serialize(report, OperationsProtocol.Json));

        Assert.Equal(HttpStatusCode.Conflict, (await f.Agent.PostAsJsonAsync(Fixture.Api + "agents/register",
            f.Capability with { BootId = "boot-b" })).StatusCode);

        var restarted = new OperationsAgentClient(f.Agent, "host-a", "boot-b", spool, f.Clock);
        await restarted.RegisterAsync(default);
        await restarted.PollOnceAsync(default);

        var completed = await f.Service.GetFromJsonAsync<OperationAttempt>(Fixture.Api + $"attempts/{dispatched.Id}");
        Assert.Equal("succeeded", completed!.State);
        Assert.Equal(3, completed.Events.Length);
        Assert.Empty(System.IO.Directory.EnumerateFiles(spool));
    }

    [Fact]
    public async Task Expired_previous_boot_evidence_is_receipted_without_accepting_a_task_result()
    {
        await using var f = await Fixture.Start();
        var spool = Path.Combine(f.Directory, "spool");
        await f.Register();
        var command = f.Command() with { Subject = new("task", "run", 4), Permit = "permit" };
        await f.Submit(command);
        var dispatched = (await f.Poll())!;
        var report = f.Report(dispatched);
        System.IO.Directory.CreateDirectory(spool);
        await File.WriteAllTextAsync(Path.Combine(spool, dispatched.Id + ".json.tmp"),
        ...
        var restarted = new OperationsAgentClient(f.Agent, "host-a", "boot-b", spool, f.Clock);
        await restarted.RegisterAsync(default);
        var original = await f.Service.GetFromJsonAsync<OperationAttempt>(Fixture.Api + $"attempts/{dispatched.Id}");
        Assert.Equal("unreachable", original!.State);
        Assert.Null(original.Result);
        Assert.Equal(OperationsProtocol.Digest(report.Result), original.LateResultDigest);
        Assert.Equal("late-result-recorded", original.Events[^1].Kind);
        Assert.Empty(System.IO.Directory.EnumerateFiles(spool));
        var resultPath = Fixture.Api + $"attempts/{dispatched.Id}/result";
        Assert.Equal(HttpStatusCode.OK, (await f.Agent.PostAsJsonAsync(resultPath, report)).StatusCode);
        Assert.Equal(HttpStatusCode.Conflict, (await f.Agent.PostAsJsonAsync(resultPath,
            report with { Result = report.Result with { ExitClassification = "changed" } })).StatusCode);

        var next = await f.Submit(f.Command("after-restart"));
        await restarted.PollOnceAsync(default);
        Assert.Equal("succeeded", (await f.Service.GetFromJsonAsync<OperationAttempt>(Fixture.Api + $"attempts/{next.Id}"))!.State);
        Assert.Equal(HttpStatusCode.Conflict, (await f.Agent.PostAsJsonAsync(Fixture.Api + $"attempts/{dispatched.Id}/result",
            report with { BootId = "boot-b" })).StatusCode);
    }

    [Fact]
    public async Task Partial_previous_boot_write_is_removed_and_new_boot_registers_after_lease_expiry()
    {
        await using var f = await Fixture.Start();
        var spool = Path.Combine(f.Directory, "spool");
        await f.Register();
        ...
        var dispatched = (await f.Poll())!;
        System.IO.Directory.CreateDirectory(spool);
        await File.WriteAllTextAsync(Path.Combine(spool, dispatched.Id + ".json.tmp"), "{\"bootId\":");
        var restarted = new OperationsAgentClient(f.Agent, "host-a", "boot-b", spool, f.Clock);

        await Assert.ThrowsAsync<HttpRequestException>(() => restarted.RegisterAsync(default));
        Assert.Empty(System.IO.Directory.EnumerateFiles(spool));
        f.Clock.Advance(TimeSpan.FromSeconds(31));
        await restarted.RegisterAsync(default);
        var original = await f.Service.GetFromJsonAsync<OperationAttempt>(Fixture.Api + $"attempts/{dispatched.Id}");
        Assert.Equal("unreachable", original!.State);
        Assert.Null(original.LateResult);
    }

    [Fact]
    public async Task Partial_temporary_report_is_removed_and_assignment_can_finish()
    {
        await using var f = await Fixture.Start();
        var spool = Path.Combine(f.Directory, "spool");
        var agent = new OperationsAgentClient(f.Agent, "host-a", "boot-a", spool, f.Clock);
    ...
    [Fact]
    public async Task Committed_report_takes_precedence_over_leftover_temporary_file()
    {
        await using var f = await Fixture.Start();
        var spool = Path.Combine(f.Directory, "spool");
        var agent = new OperationsAgentClient(f.Agent, "host-a", "boot-a", spool, f.Clock);
        ...
        await f.Submit();
        var dispatched = (await f.Poll())!;
        var committed = f.Report(dispatched) with
        {
            Result = f.Report(dispatched).Result with
            {
                Output = JsonSerializer.SerializeToElement(new { os = "committed", architecture = "x64", processors = 1 })
            }
        };
        var target = Path.Combine(spool, dispatched.Id + ".json");
        await File.WriteAllTextAsync(target, JsonSerializer.Serialize(committed, OperationsProtocol.Json));
        await File.WriteAllTextAsync(target + ".tmp", "{\"bootId\":");

        await agent.PollOnceAsync(default);

        var completed = await f.Service.GetFromJsonAsync<OperationAttempt>(Fixture.Api + $"attempts/{dispatched.Id}");
        Assert.Equal("committed", completed!.Result!.Output.GetProperty("os").GetString());
        Assert.Empty(System.IO.Directory.EnumerateFiles(spool));
    }
    ...
    public async Task Lease_loss_is_unreachable_and_never_reassigns_without_no_overlap_evidence()
    {
        ...
        var lateReport = f.Report(dispatched);
        Assert.Equal(HttpStatusCode.OK, (await f.Agent.PostAsJsonAsync(Fixture.Api + $"attempts/{dispatched.Id}/result", lateReport)).StatusCode);
        var late = await f.Service.GetFromJsonAsync<OperationAttempt>(Fixture.Api + $"attempts/{dispatched.Id}");
        Assert.Equal("unreachable", late!.State);
        Assert.Null(late.Result);
        Assert.NotNull(late.LateResult);
